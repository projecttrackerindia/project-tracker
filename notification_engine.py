...
DEFAULT_PREFS = {
    ...
    "enable_desktop": 1,
    "enable_email": 1,
    "enable_slack": 0,          # user opt-in after connecting Slack account
    "mute_after_hours": 0,
    "office_start": "09:00",
    "office_end": "18:00",
    "priority_only": 0,
    ...
    if not row:
        db.execute(
            ...
            (
                ...
                1, 1, 1, 1, 1, 1, datetime.now(timezone.utc).isoformat(),
            ),
        )
...
def update_user_prefs(db, workspace_id: str, user_id: str, payload: dict) -> dict:
    ...
    if clean:
        ...
        db.execute(
            ...
            tuple(clean.values()) + (datetime.now(timezone.utc).isoformat(), workspace_id, user_id),
        )
...
def insert_in_app(db, workspace_id: str, user_id: str, notif_type: str, content: str, entity_id: str = "", entity_type: str = "", sender_id: str = "", now: str | None = None) -> str:
    ...
    db.execute(
        ...
        (nid, workspace_id, notif_type, content, user_id, 0, now or datetime.now(timezone.utc).isoformat(), sender_id or "", entity_id or "", entity_type or ""),
    )
    return nid


def dispatch_notification(*, db, workspace_id: str, user_id: str, notif_type: str, title: str, body: str = "", priority: str = "medium", entity_id: str = "", entity_type: str = "", sender_id: str = "", url: str = "/", email_fn=None, email_args: tuple = (), push_fn=None, slack_fn=None, now: str | None = None, force_push: bool = False) -> dict:
    if not user_id:
        return {"in_app": False, "desktop": False, "email": False, "slack": False}
    prefs = get_user_prefs(db, workspace_id, user_id)
    if not allows_notification(prefs, notif_type, priority):
        return {"in_app": False, "desktop": False, "email": False, "slack": False, "muted": True}
    sent = {"in_app": False, "desktop": False, "email": False, "slack": False}
    ...
    if int(prefs.get("enable_in_app", 1) or 0):
        ...
        sent["in_app"] = True
    if push_fn and int(prefs.get("enable_desktop", 1) or 0) and (force_push or (priority or "").lower() in URGENT_PRIORITIES):
    ...
    if email_fn and int(prefs.get("enable_email", 1) or 0):
        ...
        sent["email"] = True
    if slack_fn and int(prefs.get("enable_slack", 0) or 0):
        threading.Thread(target=slack_fn, args=(workspace_id, user_id, title, body or title, url), daemon=True).start()
        sent["slack"] = True
    return sent
    ...
    while True:
        try:
            ...
            with app.get_db() as db:
                # FIX 23: Single JOIN query replaces N+1 per-task notification check
                cutoff_ts = (now - timedelta(hours=20)).isoformat()
                rows = db.execute(
                    """SELECT t.* FROM tasks t
                       LEFT JOIN notifications n ON n.workspace_id=t.workspace_id
                           AND n.user_id=t.assignee AND n.type='deadline'
                           AND n.entity_id=t.id AND n.ts>=?
                       WHERE t.due!='' AND t.assignee!=''
                         AND t.stage NOT IN ('completed','done','closed','cancelled')
                         AND t.due>=? AND t.due<=?
                         AND n.id IS NULL
                       LIMIT 200""",
                    (cutoff_ts, now.strftime("%Y-%m-%dT%H:%M:%S"), soon.strftime("%Y-%m-%dT%H:%M:%S")),
                ).fetchall()
                for t in rows:
            ...
            time.sleep(30)  # back-off on error before retry to avoid CPU spin
            continue
        time.sleep(1800)
            ...
            with app.get_db() as db:
                ...
                for t in rows:
                    exists = db.execute(
                        ...
                        (t["workspace_id"], t["id"], (datetime.now(timezone.utc) - timedelta(hours=20)).isoformat()),
                    ).fetchone()
                    if exists:
                        continue
