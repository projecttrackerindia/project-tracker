...
const _apiEtagCache = {}; // legacy only; critical JSON uses no-store
const _apiErrorSeen = new Map();
const _apiInflight = new Map(); // GET request dedupe; prevents tab-click API stampedes
...
const _apiRequest = async (u, opts = {}) => {
  ...
  try {
    ...
    // Do not send If-None-Match for app JSON. 304 responses have no body and caused blank dashboards.
    if (false && cached && cached.etag) headers['If-None-Match'] = cached.etag;
    const cleanOpts = { ...opts };
    delete cleanOpts.timeoutMs;
    if(method==='GET'){headers['Cache-Control']=headers['Cache-Control']||'no-store';headers['Pragma']=headers['Pragma']||'no-cache';}
    ...
    if (method === 'GET') {
      const cleanUrl=String(u||'').split('?')[0];
      if(['/api/app-data','/api/tickets','/api/timelogs','/api/timesheet/weeks/current','/api/workspace-os/summary','/api/workspace/roles','/api/workspace/plan-usage',
  '/api/bootstrap',
  '/api/workspace-os/bootstrap',
  '/api/dm/summary',
  '/api/activity/summary'].some(p=>cleanUrl===p||cleanUrl.startsWith(p+'/'))) ptInstantCacheSet(u,data);
    }
    return data;
  } catch (e) {
...
function Av({u,size=32}){
  ...
  const imgSrc=(u&&u.avatar_data&&u.avatar_data.startsWith('data:image'))?u.avatar_data:
               ...
               (u&&u.has_avatar&&u.id)?('/api/users/'+encodeURIComponent(u.id)+'/avatar?v='+encodeURIComponent(u.avatar_rev||u.updated||u.profile_updated||u.last_active||'live')):null;
  // Always render an initials fallback first, then layer the image over it.
  // Previously failed/slow avatar requests left a blank circle in Org Chart/Profile.
  return html`<div class="av" style=${{width:size,height:size,position:'relative',overflow:'hidden',borderRadius:'50%',background:(u&&u.color)||'#2563eb',color:'#fff',fontSize:Math.max(9,Math.floor(size*.33)),display:'inline-flex',alignItems:'center',justifyContent:'center',fontWeight:800,flex:'0 0 auto'}}>
    <span>${initials}</span>
    ${imgSrc?html`<img src=${imgSrc} style=${{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',borderRadius:'50%',background:(u&&u.color)||'#2563eb'}} onError=${e=>{try{e.currentTarget.remove();}catch(_){}}}/>`:null}
  </div>`;
}
function SP({s}){
  ...
  render(){
    if(this.state.err)return html`
      <div style=${{padding:40,textAlign:'center',color:'var(--rd)',maxWidth:520,margin:'0 auto'}}>
        ...
        <div style=${{fontSize:12,color:'var(--rd)',fontFamily:'monospace',background:'rgba(248,113,113,.08)',padding:'8px 10px',borderRadius:8,marginBottom:16,textAlign:'left',wordBreak:'break-word',maxHeight:120,overflowY:'auto'}}>
    ...
    return html`
      <div style=${{width:310,background:'var(--sf)',borderRight:'1px solid var(--bd)',display:'flex',flexDirection:'column',height:'100vh',flexShrink:0,overflow:'hidden'}}>
          ...
          <div style=${{flex:1,minWidth:0}}>
            ...
            ${lead?html`<div style=${{fontSize:9,color:'var(--tx3)'}}>Lead: <b style=${{color:'var(--cy)'}}>${lead.name}</b></div>`:
              html`<div style=${{fontSize:9,color:'var(--tx3)'}}>${members.length} members</div>`}
          </div>
    ...
    <div style=${{width:240,background:'var(--sf)',borderRight:'1px solid var(--bd)',display:'flex',flexDirection:'column',height:'100vh',flexShrink:0}}>
      <div style=${{padding:'12px 14px',borderBottom:'1px solid var(--bd)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <div>
          ...
          ${activeTeam?html`<div style=${{fontSize:10,color:'var(--ac)',marginTop:2}}>Viewing: <b>${activeTeam.name}</b></div>`:html`<div style=${{fontSize:9,color:'var(--tx3)',marginTop:2}}>All workspace data</div>`}
        </div>
      ...
      <div style=${{flex:1,overflowY:'auto',padding:'6px'}}>
        ...
        ${filtered.map(team=>{
          ...
          return html`
            <div key=${team.id}
              style=${{padding:'8px 10px',borderRadius:10,border:'2px solid '+(teamCtx===team.id?'var(--ac)':'var(--bd)'),marginBottom:7,background:teamCtx===team.id?'rgba(90,140,255,.06)':'var(--sf2)',cursor:'pointer',transition:'all .12s'}}
              onClick=${()=>{
              ...
              ${lead?html`<div style=${{fontSize:9,color:'var(--tx3)',marginBottom:6}}>Lead: <b style=${{color:'var(--cy)'}}>${lead.name}</b></div>`:null}
...
function Header({title,sub,dark,setDark,extra,cu,setCu,upcomingReminders,onViewReminders,notifs,onNotifClick,onMarkAllRead,onClearAll,activeTeam,teams,setTeamCtx}){
  ...
  const [profileFullMode,setProfileFullMode]=useState(false);
  ...
  const profileLocalKey=cu&&cu.id?('pt_profile_local_'+cu.id):'';
  const mergeSelfProfile=useCallback((patch)=>{
    if(!patch||!cu||!cu.id)return;
    const normalized=ptNormalizeProfilePayload(patch,cu);
    setCu&&setCu(prev=>ptMergeProfileNonEmpty(prev||{},normalized));
    setProfileDraft(prev=>ptMergeProfileNonEmpty(prev||{},normalized));
    try{localStorage.setItem(profileLocalKey,JSON.stringify(ptMergeProfileNonEmpty(JSON.parse(localStorage.getItem(profileLocalKey)||'{}'),normalized)));}catch(_){ }
  },[cu&&cu.id,profileLocalKey,setCu]);
  useEffect(()=>{
    if(!showNP)return;
  ...
  useEffect(()=>{
    let local={};try{local=profileLocalKey?JSON.parse(localStorage.getItem(profileLocalKey)||'{}'):{};}catch(_){local={};}
    const merged=ptMergeProfileNonEmpty(cu||{},local||{});
    setProfileBirthDate(merged.birth_date||merged.birthday||merged.dob||'');
    setProfileBirthVisibility(merged.birth_date_visibility||merged.dob_visibility||'private');
    setProfileDraft(prev=>ptMergeProfileNonEmpty(prev||{}, {
      ...merged,
      name:merged.name||'',
      email:merged.email||'',
      role:merged.role||'',
      birth_date:merged.birth_date||merged.birthday||merged.dob||'',
      birth_date_visibility:merged.birth_date_visibility||merged.dob_visibility||'private'
    }));
    if(local&&Object.keys(local).length){setCu&&setCu(prev=>ptMergeProfileNonEmpty(prev||{},local));}
  },[cu&&cu.id,cu&&cu.name,cu&&cu.email,cu&&cu.role,cu&&cu.birth_date,cu&&cu.birth_date_visibility,profileLocalKey]);
  useEffect(()=>{
    const h=e=>{const d=e&&e.detail;if(!d||!cu||String(d.user_id||d.id||cu.id)!==String(cu.id))return;mergeSelfProfile(d.profile||d);};
    window.addEventListener('pt:profile-updated',h);window.addEventListener('pt:wos-profile-sync',h);
    return()=>{window.removeEventListener('pt:profile-updated',h);window.removeEventListener('pt:wos-profile-sync',h);};
  },[cu&&cu.id,mergeSelfProfile]);
  useEffect(()=>{
    if(!showProfile||!cu||!cu.id)return;
    let cancel=false;
    api.get('/api/profile/summary',{quiet:true,timeoutMs:5000}).then(r=>{
      if(cancel||!(r&&r.ok&&r.profile))return;
      setProfileDraft(prev=>ptMergeProfileNonEmpty(prev||{}, {...r.profile,name:r.profile.name||(cu&&cu.name)||'',email:r.profile.email||(cu&&cu.email)||'',birth_date:r.profile.birth_date||r.profile.dob||(cu&&cu.birth_date)||'',birth_date_visibility:r.profile.birth_date_visibility||(cu&&cu.birth_date_visibility)||'private'}));
    }).catch(()=>{});
    return()=>{cancel=true;};
  },[showProfile,cu&&cu.id]);
  const saveFullProfile=async()=>{
    if(!cu||!cu.id)return;
    setProfileSaving(true);setProfileMsg('Saving...');
    const cleanDate=v=>/^\d{4}-\d{2}-\d{2}$/.test(String(v||''))?String(v):'';
    const birth=cleanDate(profileDraft.birth_date||profileDraft.dob||profileBirthDate||'');
    const birthVis=profileDraft.birth_date_visibility||profileBirthVisibility||'private';
    const orgPayload={...profileDraft,date_of_joining:cleanDate(profileDraft.date_of_joining),dob:birth,dob_visibility:profileDraft.dob_visibility||birthVis};
    const userPayload={name:profileDraft.name||cu.name,birth_date:birth,birth_date_visibility:birthVis};
    try{
      const a=await api.put('/api/profile/summary',{...orgPayload,...userPayload},{quiet:true,timeoutMs:15000}).catch(e=>({error:e.message}));
      if((a&&a.ok)!==false && !(a&&a.error)){
        const savedProfile=ptNormalizeProfilePayload({...profileDraft,...(a.profile||a.item||{}),name:userPayload.name,birth_date:birth,birthday:birth,birth_date_visibility:birthVis,dob_visibility:birthVis},cu||{});
        mergeSelfProfile(savedProfile);
        try{window.dispatchEvent(new CustomEvent('pt:profile-updated',{detail:{user_id:cu.id,profile:savedProfile}}));window.dispatchEvent(new CustomEvent('pt:wos-profile-sync',{detail:{user_id:cu.id,profile:savedProfile}}));window.dispatchEvent(new CustomEvent('pt:wos-refresh-soft',{detail:{reason:'profile-save'}}));}catch(_){ }
        setProfileBirthDate(birth);setProfileBirthVisibility(birthVis);setProfileMsg('✓ Profile saved');try{window._pfToast&&window._pfToast('success','Profile saved','Your changes are now synced across Profile, Workspace OS and Org Chart.');}catch(_){}setTimeout(()=>{setProfileMsg('');setProfileFullMode(false);},650);
      }else setProfileMsg((a&&a.error)||'Save failed');
          ...
          ${cu?html`<div style=${{position:'relative'}} ref=${prRef}>
            ...
            ${showProfile?html`
              <div style=${{position:'fixed',top:60,right:16,width:300,background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:18,boxShadow:'0 8px 40px rgba(0,0,0,.18)',zIndex:9500,overflow:'hidden'}}>
                <div style=${{padding:'20px 16px',background:'linear-gradient(135deg,rgba(29,78,216,.10),rgba(124,58,237,.05))',borderBottom:'1px solid var(--bd)',display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                  ...
                  <input ref=${prImgRef} type="file" accept="image/*" style=${{display:'none'}} onChange=${async e=>{
                    ...
                    reader.onload=async ev=>{
                      ...
                      const rev=Date.now();
                      const avatarPatch={avatar_data:dataUrl,has_avatar:1,avatar_rev:rev,updated:rev};
                      mergeSelfProfile(avatarPatch);
                      try{window.dispatchEvent(new CustomEvent('pt:profile-updated',{detail:{user_id:cu.id,profile:avatarPatch}}));window.dispatchEvent(new CustomEvent('pt:wos-profile-sync',{detail:{user_id:cu.id,profile:avatarPatch}}));}catch(_){ }
                      setUploadMsg('Saving photo...');
                      api.put('/api/profile',{avatar_data:dataUrl},{quiet:true}).then(res=>{
                        if(res&&res.error){setUploadMsg('Upload failed. Try a smaller image.');}
                        else{const serverProfile=ptNormalizeProfilePayload((res&&res.profile)||res||{},avatarPatch);mergeSelfProfile(ptMergeProfileNonEmpty(avatarPatch,serverProfile));try{window.dispatchEvent(new CustomEvent('pt:profile-updated',{detail:{user_id:cu.id,profile:ptMergeProfileNonEmpty(avatarPatch,serverProfile)}}));window.dispatchEvent(new CustomEvent('pt:wos-profile-sync',{detail:{user_id:cu.id,profile:ptMergeProfileNonEmpty(avatarPatch,serverProfile)}}));}catch(_){ }setUploadMsg('✓ Photo updated!');}
                      }).catch(()=>setUploadMsg('Saved locally. Server retry on refresh.'));
                ...
                ${profileMsg?html`<div style=${{padding:'10px 14px',borderBottom:'1px solid var(--bd)',fontSize:11,textAlign:'center',fontWeight:800,color:profileMsg.startsWith('✓')?'var(--gn)':profileMsg==='Saving...'?'var(--tx3)':'var(--rd)'}}>${profileMsg}</div>`:null}
                <div style=${{padding:'12px 14px',borderBottom:'1px solid var(--bd)'}}>
                  <${PersonalTwoFAToggle} cu=${cu} setCu=${setCu}/>
                </div>
                <div style=${{padding:'8px 10px'}}>
                  <p style=${{fontSize:9,color:'var(--tx3)',textAlign:'center',marginBottom:8,fontFamily:'monospace'}}>Click avatar to change profile photo</p>
                  <button class="btn bp" style=${{width:'100%',justifyContent:'center',fontSize:12,marginBottom:8}} onClick=${()=>{setShowProfile(false);setProfileFullMode(true);}}>Open complete profile</button>
                  <button class="btn bg" style=${{width:'100%',justifyContent:'center',fontSize:12}} onClick=${()=>setShowProfile(false)}>Close</button>
                </div>
              </div>`:null}
              ${profileFullMode?html`<div class="wos-overlay" style=${{position:'fixed',inset:0,zIndex:99998,background:'rgba(4,7,18,.72)',backdropFilter:'blur(10px)'}} onClick=${()=>setProfileFullMode(false)}></div><aside class="wos-drawer" style=${{position:'fixed',top:'4vh',bottom:'4vh',left:'50%',right:'auto',transform:'translateX(-50%)',width:'min(1180px,92vw)',zIndex:99999,background:'var(--wos-card)',border:'1px solid var(--wos-line)',borderRadius:22,padding:18,boxShadow:'0 30px 90px rgba(0,0,0,.55)',overflow:'auto'}} onClick=${e=>e.stopPropagation()}>
                <div class="wos-dhead"><div class="wos-profile-title"><${Av} u=${cu} size=${52}/><div><h3 style=${{margin:0}}>Complete Profile</h3><p style=${{margin:'3px 0 0',color:'var(--wos-muted)',fontSize:12}}>Single profile used across Profile, Workspace OS, Org Chart, attendance, leave and payslips.</p></div></div><button class="wos-btn" onClick=${()=>setProfileFullMode(false)}>×</button></div>
                <div class="wos-profile-tabs"><button class=${profileFullTab==='overview'?'active':''} onClick=${()=>setProfileFullTab('overview')}>Overview</button><button class=${profileFullTab==='personal'?'active':''} onClick=${()=>setProfileFullTab('personal')}>Personal Details</button><button class=${profileFullTab==='employment'?'active':''} onClick=${()=>setProfileFullTab('employment')}>Employment Details</button><button class=${profileFullTab==='security'?'active':''} onClick=${()=>setProfileFullTab('security')}>Security</button></div>
                <div class="wos-grid" style=${{maxWidth:'none'}}>
                  <div class="wos-card span-6"><h3>Account</h3><p>Login and basic identity. Birthday is managed only here, not in the quick popup.</p><div class="wos-edit-grid" style=${{marginTop:12}}><label style=${{fontSize:11,fontWeight:900,color:'var(--wos-muted)',textTransform:'uppercase'}}>Full name<input class="inp" placeholder="Full name" value=${profileDraft.name||''} onInput=${e=>setProfileDraft({...profileDraft,name:e.target.value})} style=${{marginTop:6}}/></label><label style=${{fontSize:11,fontWeight:900,color:'var(--wos-muted)',textTransform:'uppercase'}}>Email ID<input class="inp" disabled value=${profileDraft.email||cu.email||''} style=${{marginTop:6}}/></label><label style=${{fontSize:11,fontWeight:900,color:'var(--wos-muted)',textTransform:'uppercase'}}>Date of birth<input class="inp" type="date" value=${profileDraft.birth_date||profileDraft.dob||profileBirthDate||''} onInput=${e=>setProfileDraft({...profileDraft,birth_date:e.target.value,dob:e.target.value})} style=${{marginTop:6}}/></label><label style=${{fontSize:11,fontWeight:900,color:'var(--wos-muted)',textTransform:'uppercase'}}>Birthday visibility<select class="sel" value=${profileDraft.birth_date_visibility||profileBirthVisibility||'private'} onChange=${e=>setProfileDraft({...profileDraft,birth_date_visibility:e.target.value,dob_visibility:e.target.value})} style=${{marginTop:6}}><option value="private">Private</option><option value="team">Team celebration</option><option value="everyone">Visible to everyone</option></select></label></div></div>
...
function FileAttachments({taskId,projectId,readOnly}){
  ...
  return html`<div style=${{display:'flex',flexDirection:'column',gap:10}}>
    ...
    ${files.map(f=>html`
      <div key=${f.id} style=${{display:'flex',alignItems:'center',gap:10,padding:'7px 10px',background:'var(--sf2)',borderRadius:9,border:'1px solid var(--bd)'}}>
          ...
          <div style=${{display:'flex',flexDirection:'column',gap:10}}>
            ${cmts.length>0?html`<div style=${{display:'flex',flexDirection:'column',gap:8,maxHeight:240,overflowY:'auto'}}>
              ${cmts.map((c,i)=>{
                ...
                return html`<div key=${i} style=${{display:'flex',gap:9,padding:'7px 10px',background:'var(--sf2)',borderRadius:9,border:'1px solid var(--bd)'}}>
                  <${Av} u=${au} size=${24}/>
                  <div style=${{flex:1}}>
        ...
        <div style=${{padding:'20px 24px 0',flexShrink:0}}>
          <div style=${{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:14}}>
            ...
            <div style=${{display:'flex',gap:7,flexShrink:0}}>
              ${cu&&cu.role!=='Viewer'&&!edit?html`<button class="btn bg" style=${{fontSize:11,padding:'5px 10px'}} onClick=${()=>setEdit(true)}>✏ Edit</button>`:null}
              ...
              ${cu&&(cu.role==='Admin'||cu.role==='Manager')&&!edit?html`<button class="btn brd" style=${{fontSize:11,padding:'5px 10px'}} onClick=${delProject}>🗑</button>`:null}
              <button class="btn bg" style=${{padding:'7px 10px'}} onClick=${onClose}>✕</button>
            </div>
          </div>
          ${edit?html`
            ...
            ${stageGroups.map(g=>{
              ...
              return html`<div key=${g.s} style=${{marginBottom:18}}>
                <div style=${{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                  ...
                  <span style=${{fontSize:9,color:'var(--tx3)',background:'var(--bd)',padding:'1px 6px',borderRadius:4,fontFamily:'monospace'}}>${g.tasks.length}</span>
                </div>
                ${g.tasks.map(tk=>{
                  ...
                  return html`<div key=${tk.id} class="tkc" style=${{marginBottom:7,display:'flex',gap:10,alignItems:'center'}} onClick=${()=>setEditTask(tk)}>
                    ...
                    <div style=${{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:5,flexShrink:0}}>
                      ${au?html`<${Av} u=${au} size=${24}/>`:html`<div style=${{width:24,height:24,borderRadius:'50%',background:'var(--bd)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,color:'var(--tx3)'}}>?</div>`}
              ...
              ${events.map((ev,i)=>{
                ...
                return html`<div key=${i} style=${{display:'flex',gap:10,alignItems:'flex-start',padding:'8px 4px',borderBottom:'1px solid var(--bd)'}}>
                  ...
                  <div style=${{flex:1,minWidth:0}}>
                    ...
                    <div style=${{fontSize:9,color:'var(--tx3)',marginTop:2}}>${ev.ts?new Date(ev.ts).toLocaleString():''}</div>
                  </div>
                </div>`;
              })}
            </div>`}
          ...
          ${tab==='members'?html`
            <div style=${{display:'flex',flexDirection:'column',gap:8}}>
              <div style=${{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                ...
                ${cu&&cu.role!=='Viewer'?html`<button class="btn bg" style=${{fontSize:11,padding:'5px 10px'}} onClick=${()=>{setEdit(true);setTab('tasks');}}>Edit Members</button>`:null}
              </div>
              ...
              return html`
                <div key=${p.id}
                  ...
                  onMouseLeave=${e=>e.currentTarget.style.background='var(--sf)'}>
                    ...
                    <div style=${{fontSize:9,color:'var(--tx3)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginTop:1}}>${p.description||'—'}</div>
                  </div>
                                    <div style=${{display:'flex',alignItems:'center',gap:5}}>
      ...
      <div style=${{padding:'8px 18px',borderBottom:'1px solid var(--bd)',background:'var(--sf)',flexShrink:0}}>
        <div style=${{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          ...
          <div style=${{display:'flex',gap:0,flex:'0 0 auto',borderRadius:100,overflow:'hidden',border:'1px solid var(--bd)'}}>
            <button style=${{fontSize:11,padding:'5px 10px',background:'transparent',border:'none',borderRight:'1px solid var(--bd)',cursor:'pointer',color:'var(--tx2)',fontWeight:600,display:'inline-flex',alignItems:'center',gap:5,transition:'background .12s'}}
            ...
            <button onClick=${exportTasksCsv} style=${{fontSize:11,padding:'5px 10px',background:'transparent',border:'none',cursor:'pointer',color:'var(--tx2)',fontWeight:600,display:'inline-flex',alignItems:'center',gap:5,transition:'background .12s'}}
                    ...
                    <tr key=${tk.id} style=${{borderBottom:i<sorted.length-1?'1px solid var(--bd)':'none'}}
                      ...
                      <td style=${{padding:'9px 11px',minWidth:100}}>
                        <div style=${{display:'flex',alignItems:'center',gap:7}}>
                          ...
                          <span style=${{fontSize:9,color:'var(--tx3)',fontFamily:'monospace',width:28,textAlign:'right',fontWeight:700}}>${tk.pct}%</span>
                        </div>
                      </td>
...
/* ─── Dashboard ───────────────────────────────────────────────────────────── */
function Dashboard({cu,tasks,projects,users,onNav,activeTeam,teams,setTeamCtx,tickets:propTickets,dashboardSummary}){
  const t=safe(tasks);const p=safe(projects);const u=safe(users);
  const dashCounts=(dashboardSummary&&dashboardSummary.counts)||{};
  const count=(key,fallback)=>{const v=dashCounts&&dashCounts[key];return (v!==undefined&&v!==null&&v!==''&&!Number.isNaN(Number(v)))?Number(v):fallback;};
  const isAdminManager=hasOpsAccess(cu);
  ...
  return html`
    <div class="fi" style=${{height:'100%',overflowY:'auto',padding:'12px 20px',display:'flex',flexDirection:'column',gap:12}}>
      <div style=${{padding:'8px 10px',background:'var(--sf)',borderRadius:12,border:'1px solid var(--bd2)',display:'flex',alignItems:'center',gap:10}}>
        <${Av} u=${cu} size=${32}/>
        <div style=${{flex:1,minWidth:0}}>
          <div style=${{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
        ...
        ${stats.map((s,i)=>html`
          <div key=${i} onClick=${()=>onNav(s.nav)}
            style=${{background:'var(--sf)',borderRadius:16,padding:'8px 10px',position:'relative',overflow:'hidden',cursor:'pointer',transition:'all .16s',border:'1px solid var(--bd2)'}}
      ...
      <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:14,padding:'14px 18px',marginTop:4}}>
        <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
          ...
          <span style=${{fontSize:11,color:'var(--ac)',cursor:'pointer',fontWeight:600}} onClick=${()=>onNav('workspace-os:time')}>Open Workspace OS timesheet →</span>
        </div>
          ...
          ${[['all','All','#1d4ed8','rgba(29,78,216,0.08)',counts.total], ['on-track','On Track','var(--gn)','rgba(74,222,128,.1)',counts['on-track']], ['warning','At Risk','var(--am)','rgba(251,191,36,.1)',counts['warning']], ['at-risk','Needs Attn','var(--rd)','rgba(248,113,113,.1)',counts['at-risk']], ['overdue','Overdue','var(--rd)','rgba(248,113,113,.15)',counts['overdue']], ['no-dates','No Dates','var(--tx3)','rgba(255,255,255,.04)',counts['no-dates']], ].map(([k,lbl,color,bg,cnt])=>html`
            ...
            <input class="inp" placeholder="Search projects..." value=${search}
              ...
              onInput=${e=>setSearch(e.target.value)}/>
                    ...
                    <span style=${{fontSize:9,color:'var(--tx3)',fontWeight:700,textTransform:'uppercase',letterSpacing:.5}}>Sort:</span>
          ...
          return html`
            <div key=${proj.id} style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12, padding:'13px 17px',borderLeft:'4px solid '+proj.color,transition:'all .15s',cursor:'pointer'}}
              ...
              <div style=${{display:'flex',alignItems:'center',gap:10,marginBottom:proj.totalDays!==null?10:4}}>
                ...
                <span style=${{fontSize:9,color:'var(--tx3)'}}>📋 ${proj.doneTasks}/${proj.taskCount}</span>
              </div>
...
function ProductivityView({cu,tasks,projects,users,dashboardSummary}){
  const t=safe(tasks);const p=safe(projects);const u=safe(users);
  const dashCounts=(dashboardSummary&&dashboardSummary.counts)||{};
  const count=(key,fallback)=>{const v=dashCounts&&dashCounts[key];return (v!==undefined&&v!==null&&v!==''&&!Number.isNaN(Number(v)))?Number(v):fallback;};
  const now=new Date();now.setHours(0,0,0,0);
        ...
        <div style=${{flexShrink:0,display:'flex',gap:0,borderBottom:'1px solid var(--bd)',background:'var(--sf2)'}}>
          ...
          <div style=${{padding:'16px 20px',display:'flex',flexDirection:'column',gap:14}}>
            <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:'16px 20px'}}>
              ...
              <p style=${{fontSize:9,color:'var(--tx3)',marginTop:8,textAlign:'center'}}>All ${filtered.length} developers shown — horizontal bars scale with task count</p>
            </div>
          </div>`:null}
          ...
          <div style=${{padding:'14px 18px',display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style=${{flex:1,minWidth:100}}>
                ...
                <div style=${{fontSize:10,color:'var(--tx2)',marginTop:2}}>${selDev.role||'Team Member'}</div>
              </div>
              <div style=${{width:58,height:58,borderRadius:'50%',border:'3px solid '+selDev.scoreColor, display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column', background:'rgba(255,255,255,.03)',flexShrink:0}}>
                <span style=${{fontSize:16,fontWeight:900,color:selDev.scoreColor,fontFamily:'monospace',lineHeight:1}}>${selDev.score}</span>
            ...
            <div style=${{display:'flex',flexDirection:'column',gap:6}}>
              ${selTasks.map(tk=>{
                ...
                return html`
                  <div key=${tk.id} style=${{display:'flex',gap:10,padding:'7px 10px',background:'var(--sf)',borderRadius:9,border:'1px solid var(--bd)',alignItems:'center'}}>
                    ...
                    <div style=${{flex:1,minWidth:0}}>
                      ...
                      <div style=${{display:'flex',gap:5,marginTop:3,flexWrap:'wrap',alignItems:'center'}}>
                        ...
                        ${proj?html`<span style=${{fontSize:9,color:'var(--tx3)',display:'flex',alignItems:'center',gap:3}}>
...
function renderChatContent(text){
  ...
  if(fileUrl&&isVoice){
    return `<div style="display:flex;align-items:center;gap:10px;min-width:230px"><span style="width:30px;height:30px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;background:#050505;color:#fff"><span style="display:inline-flex;gap:2px;align-items:center"><i style="width:3px;height:8px;background:#fff;border-radius:3px;display:block"></i><i style="width:3px;height:14px;background:#fff;border-radius:3px;display:block"></i><i style="width:3px;height:8px;background:#fff;border-radius:3px;display:block"></i></span></span><audio controls preload="metadata" src="${fileUrl}" style="height:32px;max-width:190px"></audio></div>`;
  }
  if(fileUrl&&isImg){
    ...
    <div style=${{width:360,maxWidth:'92vw',border:'1px solid rgba(239,68,68,.42)',borderRadius:24,padding:22,background:'linear-gradient(145deg,rgba(15,23,42,.98),rgba(30,41,59,.96))',boxShadow:'0 30px 90px rgba(0,0,0,.65)',textAlign:'center'}}>
      ...
      <div style=${{display:'flex',gap:10,justifyContent:'center'}}>
        ...
        <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:7}}>
          ...
          <span style=${{fontSize:9,color:'var(--tx3)'}}>${sortedProjects.length} of ${allProjects.length}</span>
        </div>
                <div style=${{position:'relative'}}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
        ...
        <div style=${{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <div style=${{padding:'7px 10px',borderBottom:'1px solid var(--bd)',display:'flex',alignItems:'center',gap:9,flexShrink:0}}>
        ${sp?html`
        ...
        <div style=${{padding:'12px 16px',background:'var(--sf2)',borderBottom:'1px solid var(--bd)',flexShrink:0}}>
          <div style=${{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:10,marginBottom:12}}>
            ${[
              ...
              <div key=${s.label} style=${{background:'var(--sf)',borderRadius:9,padding:'8px 10px',border:'1px solid var(--bd)'}}>
                <div style=${{fontSize:20,fontWeight:800,color:s.color,lineHeight:1}}>${s.val}</div>
                <div style=${{fontSize:9,color:'var(--tx3)',marginTop:3}}>${s.label}</div>
              </div>`)}
          </div>
          <div style=${{marginBottom:10}}>
            <div style=${{display:'flex',justifyContent:'space-between',marginBottom:4}}>
          ...
          <div style=${{display:'flex',alignItems:'center',gap:6}}>
            ...
            <div style=${{display:'flex',gap:-4}}>
              ...
              ${projMembers.length>8?html`<span style=${{fontSize:9,color:'var(--tx3)',marginLeft:6}}>+${projMembers.length-8} more</span>`:null}
            </div>
          </div>
        </div>`:null}
            ...
            if(isSystem) return html`
              <div key=${m.client_msg_id||m.id} style=${{display:'flex',justifyContent:'center',padding:'3px 0',marginBottom:6}}>
                <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:3,maxWidth:'90%'}}>
                  ...
                  <span style=${{fontSize:9,color:'var(--tx3)',fontFamily:'monospace',letterSpacing:.2}}>${timeStr}</span>
                </div>
              </div>`;
        ...
        ${loadingChannel!==pid&&msgs.length===0?html`<div style=${{textAlign:'center',paddingTop:48,color:'var(--tx3)',fontSize:13}}>
          ...
          <p>No messages yet. Task activity will appear here automatically.</p>
            ...
            <div style=${{padding:'8px 10px',borderTop:'1px solid var(--bd)',display:'flex',gap:8,flexShrink:0,alignItems:'center',position:'relative'}}>
  ...
  return html`<style>@keyframes dmMenuPop{from{opacity:0;transform:translateY(-6px) scale(.94)}to{opacity:1;transform:translateY(0) scale(1)}} @keyframes dmPickerPop{from{opacity:0;transform:translateY(8px) scale(.92)}to{opacity:1;transform:translateY(0) scale(1)}} @keyframes dmBubbleIn{from{opacity:0;transform:translateY(8px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}} @keyframes callPulse{0%{box-shadow:0 0 0 0 rgba(34,197,94,.45)}70%{box-shadow:0 0 0 28px rgba(34,197,94,0)}100%{box-shadow:0 0 0 0 rgba(34,197,94,0)}}</style>${outgoingMeetCall?html`<div style=${{position:'fixed',inset:0,zIndex:99998,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(2,6,23,.52)',backdropFilter:'blur(10px)'}}>
    ...
    <div style=${{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style=${{padding:'11px 16px',borderTop:'1px solid var(--bd)',display:'flex',gap:8,flexShrink:0,alignItems:'center',position:'relative'}}>
        ...
        <button title="Attach file" class="btn" style=${{padding:'7px 10px',fontSize:16,flexShrink:0}} onClick=${()=>fileInputRef.current&&fileInputRef.current.click()} disabled=${!toId||attaching}>${attaching?'…':html`<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style=${{display:'block'}}><path d="M4 7.5A2.5 2.5 0 0 1 6.5 5H10l2 2h5.5A2.5 2.5 0 0 1 20 9.5V17a2.5 2.5 0 0 1-2.5 2.5h-11A2.5 2.5 0 0 1 4 17Z"/><path d="M12 11v5"/><path d="M9.5 13.5H14.5"/></svg>`}</button>
        <div style=${{position:'relative',flexShrink:0}}>
          <button title="Emoji" class="btn" style=${{padding:'7px 10px',fontSize:16}} onClick=${()=>setShowChatEmoji(v=>!v)} disabled=${!toId}>😊</button>
          ${showChatEmoji?html`<div onClick=${ev=>ev.stopPropagation()} style=${{position:'absolute',bottom:'110%',left:0,display:'grid',gridTemplateColumns:'repeat(9,28px)',gap:4,padding:8,border:'1px solid var(--bd)',background:'var(--sf)',borderRadius:16,boxShadow:'0 18px 40px rgba(0,0,0,.35)',zIndex:40}}>
            ${CHAT_EMOJIS.map(e=>html`<button title=${e} onMouseEnter=${ev=>emojiHover(ev,true)} onMouseMove=${emojiMove} onMouseLeave=${ev=>emojiHover(ev,false)} onClick=${ev=>{ev.stopPropagation();setTxt(v=>v+e);}} style=${{border:'none',background:'transparent',borderRadius:10,fontSize:18,lineHeight:1,padding:'5px 4px',cursor:'pointer',transition:'transform .16s cubic-bezier(.2,.9,.25,1.35),filter .16s'}}> ${e}</button>`)}
          </div>`:null}
        </div>
...
function MemberRow({u,cu,i,total,reload,ROLE_COLORS,roleOptions}){
  const [showPw,setShowPw]=useState(false);
    ...
    ${tab==='teams'?html`
      ...
      <div class="mo fi" style=${{maxWidth:400}}>
        ...
        <div style=${{display:'flex',flexDirection:'column',gap:11}}>
          ...
          <select class="sel" value=${role} onChange=${e=>setRole(e.target.value)}>${(roleOptions||ROLES).map(r=>html`<option key=${r}>${r}</option>`)}</select>
          <div>
...
function TicketsView({cu,users,projects,onReload,activeTeam,initialAssignee,initialStatus,initialTicketId,onClearInitialTicket}){
  const ticketCacheKey='tickets:'+(cu&&cu.workspace_id||cu&&cu.workspace_id_from_me||'ws')+':' +(activeTeam&&activeTeam.id||'all');
  const cachedTickets=ptInstantCacheGet(ticketCacheKey,[]);
  const [tickets,setTickets]=useState(()=>Array.isArray(cachedTickets)?cachedTickets:[]);
  const [busy,setBusy]=useState(()=>!(Array.isArray(cachedTickets)&&cachedTickets.length));
...
function TwoFASettingsCard({cu}){
  ...
  return html`
    <div class="card" style=${{marginBottom:8}}>
      ...
      <div style=${{padding:'8px 10px',background:'rgba(29,78,216,0.06)',borderRadius:10,border:'1px solid rgba(29,78,216,0.15)',marginBottom:14,display:'flex',gap:12,alignItems:'flex-start'}}>
      ...
      ${isAdmin?html`
        ...
        ${!loadingTfa&&userTfaList.length>0?html`
          <div style=${{borderRadius:10,border:'1px solid var(--bd)',overflow:'hidden'}}>
            ${userTfaList.map((u,i)=>{
              ...
              return html`
                <div key=${u.id} style=${{display:'flex',alignItems:'center',gap:12,padding:'8px 10px',
                  ...
                  <div style=${{flex:1,minWidth:0}}>
                    ...
                    <div style=${{fontSize:9,color:'var(--tx3)'}}>${u.email}</div>
                  </div>
        ...
        <div style=${{padding:'8px 10px',background:'var(--sf2)',borderRadius:8,border:'1px solid var(--bd)',fontSize:12,color:'var(--tx3)',textAlign:'center'}}>
...
function WorkspaceOSPolicySettingsCard({cu}){
  ...
  const setCatalogFromText=v=>setCfg({...cfg,activity_catalog:String(v||'').split(',').map(x=>x.trim()).filter(Boolean)});
  const addCatalogItem=()=>{const name=prompt('Activity name e.g. Client Workshop, QA Review'); if(!name)return; const arr=cfg.activity_catalog||[]; if(arr.map(x=>String(x).toLowerCase()).includes(String(name).toLowerCase()))return; setCfg({...cfg,activity_catalog:[...arr,name].slice(0,60)});};
  const removeCatalogItem=(name)=>setCfg({...cfg,activity_catalog:(cfg.activity_catalog||[]).filter(x=>String(x)!==String(name))});
  const toggleMode=m=>{const arr=cfg.allowed_work_modes||[];setCfg({...cfg,allowed_work_modes:arr.includes(m)?arr.filter(x=>x!==m):[...arr,m]});};
  const toggleRole=r=>{const arr=cfg.holiday_upload_roles||[];setCfg({...cfg,holiday_upload_roles:arr.includes(r)?arr.filter(x=>x!==r):[...arr,r]});};
  ...
  return html`<div class="card" style=${{marginBottom:16,borderColor:'rgba(90,140,255,.22)'}}>
    ...
    ${!allowed?html`<div style=${{fontSize:12,color:'var(--tx3)'}}>Only Admin/Owner/HR can configure attendance policy.</div>`:html`<div style=${{display:'grid',gridTemplateColumns:'160px 1fr 1fr',gap:12,alignItems:'start'}}>
      ...
      <label><span class="lbl">Allowed work modes</span><input class="inp" value=${(cfg.allowed_work_modes||[]).join(', ')} onInput=${e=>setModesFromText(e.target.value)}/><div style=${{display:'flex',flexWrap:'wrap',gap:6,marginTop:8}}>${modeOptions.map(m=>html`<button type="button" class="btn brd" style=${{fontSize:10,padding:'4px 8px',borderColor:(cfg.allowed_work_modes||[]).includes(m)?'var(--ac)':'var(--bd)',color:(cfg.allowed_work_modes||[]).includes(m)?'var(--ac)':'var(--tx2)'}} onClick=${()=>toggleMode(m)}>${titleCase(m)}</button>`)}</div></label>
      <label><span class="lbl">Holiday upload roles</span><input class="inp" value=${(cfg.holiday_upload_roles||[]).join(', ')} onInput=${e=>setRolesFromText(e.target.value)}/><div style=${{display:'flex',flexWrap:'wrap',gap:6,marginTop:8}}>${roleOptions.map(r=>html`<button type="button" class="btn brd" style=${{fontSize:10,padding:'4px 8px',borderColor:(cfg.holiday_upload_roles||[]).includes(r)?'var(--ac)':'var(--bd)',color:(cfg.holiday_upload_roles||[]).includes(r)?'var(--ac)':'var(--tx2)'}} onClick=${()=>toggleRole(r)}>${titleCase(r)}</button>`)}</div></label>
      <label style=${{gridColumn:'1/-1'}}><span class="lbl">Timesheet activity catalog</span><input class="inp" value=${(cfg.activity_catalog||[]).join(', ')} onInput=${e=>setCatalogFromText(e.target.value)} placeholder="Meeting, Client Call, Code Review, Deployment"/><div style=${{display:'flex',flexWrap:'wrap',gap:6,marginTop:8}}>${(cfg.activity_catalog||[]).map(a=>html`<button type="button" class="btn brd" style=${{fontSize:10,padding:'4px 8px',color:'var(--tx2)',borderColor:'var(--bd)'}} onClick=${()=>removeCatalogItem(a)}>${a} ×</button>`)}<button type="button" class="btn bg" style=${{fontSize:10,padding:'4px 8px'}} onClick=${addCatalogItem}>+ Add activity</button></div><small style=${{display:'block',color:'var(--tx3)',marginTop:6}}>Reusable non-task work options used by Workspace OS → Timesheet. Users can still type a manual activity.</small></label>
    </div>`}
    ${msg?html`<div style=${{marginTop:10,fontSize:12,color:msg.includes('failed')?'var(--rd)':'var(--gn)'}}>${msg}</div>`:null}
  </div>`;
}
...
function WorkspaceOSAdminImportsCard({cu}){
  ...
  const [holiday,setHoliday]=useState({title:'',holiday_date:today.toISOString().slice(0,10),category:'Public Holiday'});
  ...
  const importHolidays=async()=>{
    if(!holidayFile){setMsg('Choose a CSV/XLSX holiday file');return;}
    const fd=new FormData();fd.append('file',holidayFile);
    setBusy('holiday-import');
    const r=await api.upload('/api/holidays/import',fd).catch(e=>({error:e.message}));
    setBusy('');
    if(r&&r.ok){setHolidayFile(null);setMsg(`Imported ${r.imported||0} holidays${r.failed?`, ${r.failed} failed`:''}`);try{window.dispatchEvent(new CustomEvent('pt:wos-refresh'));}catch(_){}}
    else setMsg((r&&r.error)||'Holiday import failed');
    setTimeout(()=>setMsg(''),3200);
  };
  const uploadPayslips=async()=>{
    const files=Array.from(payFiles||[]);
    if(!files.length){setMsg('Choose one or more payslip PDFs');return;}
    const fd=new FormData();files.forEach(f=>fd.append('files',f));if(payMap)fd.append('mapping',payMap);fd.append('month',pay.month);fd.append('year',pay.year);
    setBusy('payslips');
  ...
  return html`<div class="card" style=${{marginBottom:16,borderColor:'rgba(90,140,255,.25)',background:'linear-gradient(180deg,var(--sf),rgba(90,140,255,.035))'}} id="workspace-os-admin-imports">
    ...
    <div style=${{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)',gap:16}}>
      <div style=${dropStyle}>
        <div style=${{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,marginBottom:12}}><div><h4 style=${{fontSize:13,fontWeight:800,margin:0,color:'var(--tx)'}}>Organisation holidays</h4><p style=${{fontSize:11,color:'var(--tx3)',margin:'3px 0 0'}}>Add one holiday or import a full year calendar.</p></div><button class="btn bp" style=${{fontSize:11,padding:'7px 12px'}} onClick=${addHoliday} disabled=${!!busy}>${busy==='holiday'?'Saving…':'Add holiday'}</button></div>
        <div style=${{display:'grid',gridTemplateColumns:'1.2fr .8fr .8fr',gap:10,marginBottom:10}}><input class="inp" placeholder="Holiday title" value=${holiday.title} onInput=${e=>setHoliday({...holiday,title:e.target.value})}/><input class="inp" type="date" value=${holiday.holiday_date} onInput=${e=>setHoliday({...holiday,holiday_date:e.target.value})}/><select class="sel" value=${holiday.category} onChange=${e=>setHoliday({...holiday,category:e.target.value})}><option>Public Holiday</option><option>Optional Holiday</option><option>Company Holiday</option><option>Regional Holiday</option></select></div>
...
function WorkspaceSettings({cu,onReload}){
  ...
  return html`<div class="fi" style=${{height:'100%',overflowY:'auto',padding:'24px'}}>
    <div style=${{maxWidth:1180,width:'100%',margin:'0 auto'}}>
      ...
      <div class="card" style=${{marginBottom:8}}>
        <h3 style=${{fontSize:13,fontWeight:700,color:'var(--tx)',letterSpacing:'-0.01em',marginBottom:8}}>🏢 Workspace</h3>
        <div style=${{display:'flex',flexDirection:'column',gap:12}}>
            ...
            <tbody>
              ${Object.entries(perms).map(([label,roleMap],i)=>html`
                <tr key=${label} style=${{background:i%2===0?'transparent':'var(--sf2)'}}>
                  <td style=${{padding:'7px 10px',color:'var(--tx2)',fontWeight:500,fontSize:12}}>${label}</td>
                  ${['Admin','Manager','TeamLead','Developer','Tester','Viewer'].map(r=>html`
      ...
      <div style=${{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style=${{padding:'14px 20px',borderTop:'1px solid var(--bd)',background:'var(--sf)',flexShrink:0}}>
          ...
          ${uploadedFiles.length>0?html`
            <div style=${{display:'flex',gap:6,marginBottom:10,flexWrap:'wrap',padding:'8px 10px',background:'var(--sf2)',borderRadius:10,border:'1px solid var(--bd)'}}>
              ...
              <div style=${{fontSize:9,color:'var(--tx3)',padding:'6px 8px',display:'flex',alignItems:'center'}}>
  ...
  return html`
    <div class="fi" style=${{height:'100%',overflowY:'auto',padding:'8px 12px',background:'var(--bg)'}}>
      ...
      <div style=${{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:18}}>
        ${statCards.map(s=>{
          return html`
            <div key=${s.label} style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:10,padding:'9px 11px',display:'flex',alignItems:'center',gap:12}}>
              ...
              <button class="btn bg" style=${{padding:'7px 10px'}} onClick=${()=>{setShowAdd(false);setAddCustomTitle('');setAddTaskId('');}}>✕</button>
                ...
                <div style=${{display:'flex',alignItems:'center',gap:7,marginBottom:14}}>
                  ...
                  <div>
                    ...
                    <div style=${{fontSize:9,color:'var(--tx3)'}}>Linked to a task</div>
                  </div>
                </div>
                <div style=${{display:'flex',flexDirection:'column',gap:10}}>
                  <div>
                ...
                <div style=${{display:'flex',alignItems:'center',gap:7,marginBottom:14}}>
                  ...
                  <div>
                    ...
                    <div style=${{fontSize:9,color:'var(--tx3)'}}>Standalone note or meeting</div>
                  </div>
                </div>
                <div>
                ...
                <div style=${{marginTop:10,padding:'8px 10px',background:'rgba(167,139,250,.07)',borderRadius:8,border:'1px solid rgba(167,139,250,.18)',fontSize:11,color:'var(--tx3)',lineHeight:1.5}}>
                  💡 Use this for meetings, calls, or any non-task reminder.
                        ...
                        <div style=${{background:'var(--sf2)',borderRadius:14,padding:'16px',border:'1px solid var(--bd)',marginBottom:8}}>
              <div style=${{display:'flex',alignItems:'center',gap:7,marginBottom:14}}>
...
function TimesheetView({cu,teams,users,projects,tasks}){
  const isAdmin=cu&&(cu.role==='Admin'||cu.role==='Manager');
  const tsCacheKey='timesheet:'+(cu&&cu.workspace_id||cu&&cu.workspace_id_from_me||'ws')+':' +(cu&&cu.id||'me');
  const tsCached=ptInstantCacheGet(tsCacheKey,{logs:[],week:null,requiredHrs:8});
  const [logs,setLogs]=useState(()=>Array.isArray(tsCached.logs)?tsCached.logs:[]);
  const [weekInfo,setWeekInfo]=useState(()=>tsCached.week||null);
  const [busy,setBusy]=useState(false);
  const [loading,setLoading]=useState(()=>!(Array.isArray(tsCached.logs)&&tsCached.logs.length));
  ...
  // ── Policy
  const [requiredHrs,setRequiredHrs]=useState(()=>Number(tsCached.requiredHrs||8));
  const [adminHrsInput,setAdminHrsInput]=useState(()=>String(tsCached.requiredHrs||8));
  ...
  const load=useCallback(async()=>{
    const cached=ptInstantCacheGet(tsCacheKey,{logs:[],week:null,requiredHrs:requiredHrs});
    if(Array.isArray(cached.logs)&&cached.logs.length){setLogs(cached.logs);setLoading(false);}
    const [d,h,w]=await Promise.all([api.get('/api/timelogs',{quiet:true,timeoutMs:7000}),api.get('/api/timelogs/required-hours',{quiet:true,timeoutMs:5000}),api.get('/api/timesheet/weeks/current',{quiet:true,timeoutMs:5000}).catch(()=>null)]);
    let nextLogs=Array.isArray(d)?d:(Array.isArray(cached.logs)?cached.logs:[]);
    let nextWeek=(w&&w.week)?w.week:(cached.week||null);
    const hrs=Number((h&&h.hours!=null)?h.hours:(cached.requiredHrs||8));
    if(Array.isArray(d))setLogs(d);
    setRequiredHrs(hrs);setAdminHrsInput(String(hrs));
    if(nextWeek)setWeekInfo(nextWeek);
    ptInstantCacheSet(tsCacheKey,{logs:nextLogs,week:nextWeek,requiredHrs:hrs});
    setLoading(false);
  },[tsCacheKey]);
  ...
  // Keep timesheet fast: do not auto-refresh on browser focus.
  // Reloading on every focus was causing visible jumps and stale data flashes.
  ...
  const monthStrip=useMemo(()=>{
    const [yy,mm]=String(filterMonth||localMonth()).split('-').map(Number);
    const days=new Date(yy,mm,0).getDate();
    const by={}; logs.forEach(l=>{const k=String(l.date||''); if(k.startsWith(String(filterMonth||localMonth()))){by[k]=(by[k]||0)+toHrs(l);}});
    return Array.from({length:days},(_,i)=>{const d=i+1;const date=yy+'-'+String(mm).padStart(2,'0')+'-'+String(d).padStart(2,'0');const h=by[date]||0;const dow=new Date(yy,mm-1,d).getDay();return {d,date,h,dow};});
  },[logs,filterMonth]);
  ...
  const handleSave=async()=>{
    ...
    const tempId='tmp_'+Date.now();
    const optimisticEntry={...payload,id:tempId,user_id:cu.id,user_name:cu.name||cu.email||'Me',_saving:true};
    setLogs(prev=>{const next=[optimisticEntry,...prev];ptInstantCacheSet(tsCacheKey,{logs:next,week:weekInfo,requiredHrs});return next;});
    setForm(blankForm());
    setShowForm(false);
    ...
    try{
      ...
      setLogs(prev=>{const next=prev.filter(l=>l.id!==tempId);ptInstantCacheSet(tsCacheKey,{logs:next,week:weekInfo,requiredHrs});return next;});
  ...
  const saveEdit=async(l)=>{
    ...
    if(!h&&!m)return;
    const old=logs;
    const patch={hours:h,minutes:Math.min(59,m),comments:editForm.comments};
  ...
  return html`<div style=${{padding:'0 0 32px'}}>
    ...
    <!-- ══ Compact Timesheet Command Bar ══ -->
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:'7px 10px',marginBottom:8,display:'grid',gridTemplateColumns:'minmax(260px,1fr) auto',gap:8,alignItems:'center'}}>
      <div style=${{display:'flex',alignItems:'center',gap:10,minWidth:0,flexWrap:'wrap'}}>
        <b style=${{fontSize:12,color:'var(--tx)',whiteSpace:'nowrap'}}>Weekly</b>
        <span style=${{fontSize:10,color:'var(--tx2)',whiteSpace:'nowrap'}}>Status: <b>${weekInfo&&weekInfo.status?weekInfo.status:'draft'}</b></span>
        <span style=${{fontSize:10,color:'var(--tx2)',whiteSpace:'nowrap'}}>${weekInfo?Math.round((weekInfo.total_minutes||0)/60*10)/10:0}h total</span>
        <span style=${{fontSize:10,color:'var(--tx2)',whiteSpace:'nowrap'}}>${weekInfo?Math.round((weekInfo.billable_minutes||0)/60*10)/10:0}h billable</span>
        ${isAdmin?html`
          <span style=${{height:16,width:1,background:'var(--bd)'}}></span>
          <span style=${{fontSize:10,fontWeight:800,color:'var(--ac)',letterSpacing:'.04em'}}>POLICY</span>
          <span style=${{fontSize:10,color:'var(--tx2)'}}>hrs/day</span>
          <input type="number" min="1" max="24" step="0.5" value=${adminHrsInput}
            onChange=${e=>setAdminHrsInput(e.target.value)}
            style=${{width:48,background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 6px',color:'var(--tx)',fontSize:11,textAlign:'center',fontWeight:800}}/>
          <button class="btn bg" style=${{fontSize:10,padding:'4px 8px'}} onClick=${async()=>{
            const hrs=parseFloat(adminHrsInput)||8;
            const r=await api.post('/api/timelogs/required-hours',{hours:hrs},{quiet:true,timeoutMs:5000});
            if(r&&r.ok){setRequiredHrs(hrs);ptInstantCacheSet(tsCacheKey,{logs,week:weekInfo,requiredHrs:hrs});setSaveMsg('✓ Policy saved');setTimeout(()=>setSaveMsg(''),1800);}
          }}>Save</button>
          <span style=${{fontSize:10,color:'var(--tx3)'}}>Active <b style=${{color:'var(--tx)'}}>${requiredHrs}h</b></span>`:null}
      </div>
      <div style=${{display:'flex',gap:6,justifyContent:'flex-end',alignItems:'center',flexWrap:'wrap'}}>
        ${saveMsg?html`<span style=${{fontSize:11,color:saveMsg.startsWith('⚠')?'#ef4444':'#22c55e',fontWeight:700}}>${saveMsg}</span>`:null}
        <button class="btn bg" style=${{fontSize:10,padding:'4px 8px'}} onClick=${submitWeek}>Submit</button>
        ${isAdmin?html`<button class="btn bp" style=${{fontSize:10,padding:'4px 8px'}} onClick=${()=>approveWeek('approved')}>Approve</button><button class="btn bg" style=${{fontSize:10,padding:'4px 8px'}} onClick=${()=>approveWeek('rejected')}>Reject</button>`:null}
        <button class="btn bg" style=${{fontSize:10,padding:'4px 8px'}} onClick=${downloadCSV}>CSV${filtered.length?html` · ${filtered.length}`:null}</button>
        <button class="btn bp" style=${{fontSize:10,padding:'5px 10px'}} onClick=${()=>{setShowForm(v=>!v);setSaveMsg('');}}>${showForm?'Close':'+ Log'}</button>
      </div>
    </div>

    <!-- ══ Log Hours Form ══ -->
    ${showForm?html`
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:12,marginBottom:8,boxShadow:'0 4px 24px rgba(0,0,0,.18)'}}>
      ...
      <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(165px,1fr))',gap:8}}>
        ...
        <div style=${{gridColumn:'1/-1'}}>
          ...
          <textarea placeholder="What did you work on? (optional)" value=${form.comments}
            ...
            style=${{width:'100%',background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:8,padding:'8px 10px',color:'var(--tx)',fontSize:12,resize:'vertical',minHeight:38,fontFamily:'inherit',boxSizing:'border-box'}}></textarea>
        </div>
      </div>
      <div style=${{marginTop:12,display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        <button class="btn bp" style=${{fontSize:12,padding:'7px 16px',display:'flex',alignItems:'center',gap:6}} onClick=${handleSave} disabled=${busy}>
          ${busy?html`<span class="spin"></span>`:null} Save Log
        </button>
        <button class="btn bg" style=${{fontSize:12,padding:'7px 10px'}} onClick=${()=>{setShowForm(false);setSaveMsg('');}}>Cancel</button>
        ${saveMsg?html`<span style=${{fontSize:12,color:saveMsg.startsWith('⚠')?'#ef4444':'#22c55e',fontWeight:600}}>${saveMsg}</span>`:null}
      </div>
    </div>`:null}

    <!-- ══ Compact Filters + KPIs ══ -->
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:'6px 8px',marginBottom:8,display:'grid',gridTemplateColumns:'minmax(320px,1fr) auto',gap:8,alignItems:'center'}}>
      <div style=${{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',minWidth:0}}>
        <span style=${{fontSize:10,fontWeight:800,color:'var(--tx3)',flexShrink:0}}>Filter</span>
        ${['today','yesterday','week','month','custom'].map(m=>html`
          <button key=${m} onClick=${()=>setFilterMode(m)}
            style=${{fontSize:10,padding:'4px 9px',borderRadius:100,cursor:'pointer',fontWeight:700,transition:'all .15s',
              border:'1px solid '+(filterMode===m?'var(--ac)':'var(--bd)'),
              background:filterMode===m?'var(--ac3)':'transparent',
              color:filterMode===m?'var(--ac)':'var(--tx2)'}}>
            ${filterLabels[m]}
          </button>`)}
        ${filterMode==='month'?html`<input type="month" value=${filterMonth} onChange=${e=>setFilterMonth(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 8px',color:'var(--tx)',fontSize:10}}/>`:null}
        ${filterMode==='custom'?html`<input type="date" value=${filterFrom} onChange=${e=>setFilterFrom(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 8px',color:'var(--tx)',fontSize:10}}/><span style=${{color:'var(--tx3)',fontSize:11}}>→</span><input type="date" value=${filterTo} onChange=${e=>setFilterTo(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 8px',color:'var(--tx)',fontSize:10}}/>`:null}
        <input type="search" placeholder="Search…" value=${searchQ} onChange=${e=>setSearchQ(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 8px',color:'var(--tx)',fontSize:10,width:150}}/>
        ${isAdmin&&users&&users.length>0?html`<select value=${filterUser} onChange=${e=>setFilterUser(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:7,padding:'4px 8px',color:'var(--tx)',fontSize:10}}><option value="">All Users</option>${(users||[]).map(u=>html`<option key=${u.id} value=${u.id}>${u.name}</option>`)}</select>`:null}
      </div>
      <div style=${{display:'flex',alignItems:'center',gap:6,justifyContent:'flex-end',flexWrap:'wrap'}}>
        ${(()=>{const days=filterMode==='today'||filterMode==='yesterday'?1:filterMode==='week'?5:filterMode==='month'?22:1;const expected=days*requiredHrs;const pct=expected>0?Math.round(totalHrs/expected*100):0;const col=pct>=100?'#22c55e':pct>=70?'var(--ac)':'#ef4444';return html`
          <span style=${{fontSize:10,padding:'4px 8px',border:'1px solid var(--bd)',borderRadius:9,color:'var(--tx2)'}}>Hours <b style=${{fontSize:12,color:'var(--ac)'}}>${loading?'…':fmtHrs(totalHrs)}</b></span>
          <span style=${{fontSize:10,padding:'4px 8px',border:'1px solid var(--bd)',borderRadius:9,color:'var(--tx2)'}}>Logs <b style=${{fontSize:12,color:'var(--tx)'}}>${filtered.length}</b></span>
          <span style=${{fontSize:10,padding:'4px 8px',border:'1px solid var(--bd)',borderRadius:9,color:'var(--tx2)'}}>Avg <b style=${{fontSize:12,color:'var(--tx)'}}>${filtered.length?fmtHrs(totalHrs/filtered.length):'—'}</b></span>
          <span style=${{fontSize:10,padding:'4px 8px',border:'1px solid var(--bd)',borderRadius:9,color:'var(--tx2)'}}>Policy <b style=${{fontSize:12,color:col}}>${pct}%</b></span>`;})()}
      </div>
    </div>

    <!-- ══ Team Summary (admin) ══ -->
    ${isAdmin&&byUser.length>0?html`
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:'14px 18px',marginBottom:8}}>
      <div style=${{fontSize:12,fontWeight:700,color:'var(--tx)',marginBottom:12,display:'flex',alignItems:'center',gap:8}}>
        👥 Team Summary <span style=${{fontSize:9,color:'var(--tx3)',fontWeight:500}}>policy: ${requiredHrs}h/day</span>
      </div>
      <div style=${{display:'flex',flexDirection:'column',gap:8}}>
        ${byUser.map(u=>{
          ...
          return html`<div key=${u.name} style=${{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div style=${{fontSize:9,color:'var(--tx3)',width:30,textAlign:'right',flexShrink:0}}>${pct}%</div>
          </div>`;
        })}
      </div>
    </div>`:null}
    ...
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,padding:'10px 12px',marginBottom:8}}>
      <div style=${{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,marginBottom:8}}>
        <div style=${{fontSize:12,fontWeight:800,color:'var(--tx)'}}>Monthly log pattern <span style=${{fontSize:10,color:'var(--tx3)',fontWeight:500}}>· each day shows logged-hours intensity</span></div>
        <input type="month" value=${filterMonth} onChange=${e=>setFilterMonth(e.target.value)} style=${{background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:8,padding:'5px 8px',color:'var(--tx)',fontSize:11}}/>
      </div>
      <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(28px,1fr))',gap:6,alignItems:'center'}}>
        ${monthStrip.map(x=>{const full=x.h>=requiredHrs;const half=x.h>0&&!full;const weekend=x.dow===0||x.dow===6;return html`<button title=${x.date+' · '+fmtHrs(x.h)} onClick=${()=>{setFilterMode('custom');setFilterFrom(x.date);setFilterTo(x.date);}} style=${{height:30,borderRadius:999,border:'1px solid '+(full?'rgba(34,197,94,.5)':half?'rgba(168,85,247,.55)':weekend?'rgba(245,158,11,.3)':'var(--bd)'),background:full?'rgba(34,197,94,.22)':half?'rgba(168,85,247,.2)':weekend?'rgba(245,158,11,.12)':'transparent',color:full?'#22c55e':half?'#c084fc':weekend?'#f59e0b':'var(--tx3)',fontSize:11,fontWeight:900,cursor:'pointer'}}>${x.d}</button>`})}
      </div>
    </div>

    <!-- ══ Entries Table ══ -->
    <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:12,overflow:'hidden'}}>
      <div style=${{padding:'8px 12px',borderBottom:'1px solid var(--bd)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
      ...
      ${loading?html`<div style=${{textAlign:'center',padding:'24px',color:'var(--tx3)'}}><span class="spin"></span></div>`
      :filtered.length===0?html`
        <div style=${{textAlign:'center',padding:'30px 16px',color:'var(--tx3)'}}>
          <div style=${{fontSize:28,marginBottom:6}}>⏱</div>
          ...
          <table style=${{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            <thead>
              <tr style=${{background:'var(--sf2)'}}>
                ${isAdmin?html`<th style=${{padding:'7px 10px',textAlign:'left',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>USER</th>`:null}
                <th style=${{padding:'7px 10px',textAlign:'left',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>DATE</th>
                <th style=${{padding:'7px 10px',textAlign:'left',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>PROJECT</th>
                <th style=${{padding:'7px 10px',textAlign:'left',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>TASK</th>
                <th style=${{padding:'7px 10px',textAlign:'right',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>TIME</th>
                <th style=${{padding:'7px 10px',textAlign:'left',fontWeight:700,color:'var(--tx3)',fontSize:10,letterSpacing:'.04em'}}>COMMENTS</th>
            ...
            <tbody>
              ${filtered.map((l,i)=>{
                ...
                return html`<tr key=${l.id}
                  ...
                  ${isAdmin?html`<td style=${{padding:'7px 10px',fontWeight:600,whiteSpace:'nowrap',color:'var(--tx)',fontSize:11}}>${l.user_name||l.user_id}</td>`:null}
                  <td style=${{padding:'7px 10px',color:'var(--tx2)',whiteSpace:'nowrap',fontSize:11}}>${l.date}</td>
                  <td style=${{padding:'7px 10px',maxWidth:110,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                  ...
                  <td style=${{padding:'7px 10px',color:'var(--tx)',fontWeight:500,maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontSize:12}}>${l.task_name||'—'}</td>
                  ...
                  <td style=${{padding:'7px 10px',maxWidth:180,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:'var(--tx3)',fontSize:11}}>
                    ${isEditing?html`
...
function PasswordGeneratorView(){
  ...
  return html`<div style=${S.wrap}>
    ...
    <div style=${S.card}>
      ...
      <input class="inp" style=${{width:'100%',background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:8,padding:'7px 10px',fontSize:13,color:'var(--tx)',fontFamily:'monospace'}}
...
function VaultPwModal({title, mode, onConfirm, onClose}){
  ...
  const inp = {
    ...
    padding:'8px 42px 8px 10px',fontSize:13,color:'var(--tx)',fontFamily:'inherit',
    outline:'none',boxSizing:'border-box',
  };
  const eyeBtn = {position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:'var(--tx3)',cursor:'pointer',fontSize:11,padding:'4px 6px'};
  return html`
    <div style=${{...VS.moBack,zIndex:2147483000}} onClick=${e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style=${{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:16,padding:'26px',width:400,maxWidth:'calc(100vw - 32px)',boxShadow:'0 32px 80px rgba(0,0,0,.6)',position:'relative'}}>
        ...
        <div style=${{marginBottom:12,position:'relative'}}>
          ...
          <button style=${eyeBtn} type="button" onClick=${()=>setShow1(!show1)}>${show1?'Hide':'Show'}</button>
        </div>
        ${mode==='set' && html`
          <div style=${{marginBottom:12,position:'relative'}}>
            <input id="vlt-pw2" style=${inp} type=${show2?'text':'password'} placeholder="Confirm password" value=${pw2}
              autoComplete="new-password"
...
function workspaceSlugFromUser(u){
  try{
    // Never use a workspace id as the slug. Logs showed bad URLs like
    // /ws1777028199772/ws1777028199772/dashboard when workspace_name was absent.
    const explicit=(u&&(u.workspace_slug||u.slug||u.workspace_name_slug))||'';
    const named=(u&&(u.workspace_name||u._ws_name||u.name))||'';
    let raw=explicit||named||'workspace';
    raw=String(raw||'workspace').trim();
    if(/^ws\d+/i.test(raw))raw='workspace';
...
async function ptResolveDmNotificationTarget(cuId, dmUnread){
  ...
  try{
    const latest=await api.get('/api/dm/route-target',{quiet:true,timeoutMs:1500});
...
<style id="wos-v48-profile-polish">
  ...
  return html`<div style=${{height:'100%',minHeight:'calc(100vh - 112px)',overflow:'auto',padding:'24px 28px',background:'var(--bg)'}}>
    ...
    ${loading?html`<div style=${{marginBottom:14,padding:'8px 10px',borderRadius:12,background:'rgba(90,140,255,.10)',border:'1px solid rgba(90,140,255,.20)',color:'var(--tx2)',fontSize:12,display:'flex',gap:8,alignItems:'center'}}>⏳ Loading billing details…</div>`:null}
    ${err?html`<div style=${{marginBottom:14,padding:'8px 10px',borderRadius:12,background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.22)',color:'var(--rd)',fontSize:12,fontWeight:750,display:'flex',gap:8,alignItems:'center'}}>⚠️ ${err} <button class="btn bg" style=${{marginLeft:'auto',padding:'3px 10px',fontSize:11}} onClick=${()=>setErr('')}>✕</button></div>`:null}
      ...
      <div style=${{display:'grid',gridTemplateColumns:'minmax(0,1.3fr) minmax(260px,.7fr)',gap:16}}>
        ...
        <div style=${{padding:20,borderRadius:20,background:'var(--sf)',border:'1px solid var(--bd)'}}>
          ...
          ${(invoices||[]).slice(0,6).length?html`<div style=${{display:'flex',flexDirection:'column',gap:6}}>
            ${invoices.slice(0,6).map(inv=>html`
              <div style=${{display:'grid',gridTemplateColumns:'140px 1fr 110px 90px 34px',gap:8,alignItems:'center',padding:'8px 10px',borderRadius:12,background:'var(--sf2)',border:'1px solid var(--bd)'}}>
        ...
        <div style=${{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style=${{padding:16,borderRadius:20,background:'var(--sf)',border:'1px solid var(--bd)'}}>
            <div style=${{fontSize:12,fontWeight:900,color:'var(--tx)',marginBottom:10}}>Invoice breakdown</div>
    ...
    ${tab==='invoices'?html`<div>
      ...
      <div style=${{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
        <input class="inp" placeholder="Search invoices…" value=${searchQ} onInput=${e=>setSearchQ(e.target.value)} style=${{width:190,height:34,fontSize:13}}/>
        <div style=${{display:'flex',gap:4}}>
...
function WorkspaceRolesSettings({cu}){
  const role=normRole(cu&&cu.role);
  const canManage=['admin','owner','workspaceowner','manager','superadmin','hr','peopleops','peopleadmin'].includes(role);
  ...
  const save=async()=>{
    ...
    if(!draft.name.trim())return setMsg('Role name required');
    const r=await api.post('/api/workspace/roles',draft,{quiet:true,timeoutMs:7000}).catch(e=>({error:e.message}));
    if(r&&r.ok){setRoles(prev=>[...(prev||[]).filter(x=>String(x.name).toLowerCase()!==String(r.item.name).toLowerCase()),r.item].sort((a,b)=>String(a.name).localeCompare(String(b.name))));setDraft({name:'',base_role:'Developer',description:'',color:'#6366f1'});setMsg('Role saved. It is now available in Add Member and Org Chart.');setTimeout(()=>setMsg(''),2200);}else setMsg(r&&r.error||'Role save failed');
  };
  const css=html`<style>
    .roles-card{border:1px solid var(--bd);background:var(--sf);border-radius:18px;padding:16px;margin-bottom:8px;box-shadow:0 10px 30px rgba(0,0,0,.06)}
    .roles-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px}.roles-head h3{margin:0;color:var(--tx);font-size:14px}.roles-head p{margin:4px 0 0;color:var(--tx3);font-size:12px;line-height:1.5}
    .role-grid{display:grid;grid-template-columns:1.2fr 1fr 2fr 44px auto;gap:8px;align-items:center}.role-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-top:12px}.role-pill{border:1px solid var(--bd);background:var(--sf2);border-radius:14px;padding:10px 12px;display:flex;gap:10px;align-items:flex-start}.role-dot{width:10px;height:10px;border-radius:999px;margin-top:4px}.role-pill b{display:block;color:var(--tx);font-size:12px}.role-pill span{display:block;color:var(--tx3);font-size:11px;margin-top:2px;line-height:1.4}.roles-note{font-size:11px;color:var(--tx3);margin-top:10px}.roles-msg{font-size:12px;color:var(--ac);font-weight:800;margin-top:10px}
    @media(max-width:800px){.role-grid{grid-template-columns:1fr}.role-grid .btn{width:100%}}
  </style>`;
  return html`${css}<div class="roles-card">
    <div class="roles-head"><div><h3>🧩 Custom Roles & Permissions</h3><p>Create workspace roles in Settings. These roles appear while adding members and in the organisation chart. Keep Org Chart focused on hierarchy, not role setup.</p></div><button class="btn bg" onClick=${load}>↻ Refresh</button></div>
    ${canManage?html`<div class="role-grid"><input class="inp" placeholder="Role name e.g. HR Executive" value=${draft.name} onInput=${e=>setDraft({...draft,name:e.target.value})}/><select class="sel" value=${draft.base_role} onChange=${e=>setDraft({...draft,base_role:e.target.value})}><option>Admin</option><option>Manager</option><option>HR</option><option>TeamLead</option><option>Developer</option><option>Tester</option><option>Viewer</option></select><input class="inp" placeholder="Description / access purpose" value=${draft.description} onInput=${e=>setDraft({...draft,description:e.target.value})}/><input class="inp" type="color" value=${draft.color} onInput=${e=>setDraft({...draft,color:e.target.value})}/><button class="btn bp" onClick=${save}>Save role</button></div>`:html`<div class="roles-note">Only Admin, Manager, HR or Owner can create roles.</div>`}
    ${msg?html`<div class="roles-msg">${msg}</div>`:null}
    <div class="role-list">${(roles||[]).map(r=>html`<div class="role-pill"><i class="role-dot" style=${{background:r.color||'#6366f1'}}></i><div><b>${r.name}</b><span>${r.description||('Base access: '+(r.base_role||'Member'))}</span></div></div>`)}</div>
    <div class="roles-note">Tip: assign reporting managers and departments from Workspace OS → Org Chart. Role creation remains here in Settings.</div>
  </div>`;
}
...
function WorkspaceOSView({cu,users=[]}){
  const role=normRole(cu&&cu.role);
  const canPeopleAdmin=['admin','owner','workspaceowner','manager','superadmin','hr','peopleops','peopleadmin'].includes(role);
  const canHR=['admin','owner','workspaceowner','superadmin','hr','peopleops','peopleadmin'].includes(role);
  const cacheKey='workspace-os:v45:'+(cu&&cu.workspace_id||'ws')+':'+(cu&&cu.id||'me');
  const todayStr=()=>{const d=new Date();d.setMinutes(d.getMinutes()-d.getTimezoneOffset());return d.toISOString().slice(0,10)};
  const fmtTime=v=>{try{return v?new Date(v).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'—'}catch(_){return v||'—'}};
  ...
  const [holidayFile,setHolidayFile]=useState(null);
  const [policy,setPolicy]=useState({title:'',category:'Policy',body:'',requires_ack:false});
  const [ann,setAnn]=useState({title:'',body:'',priority:'normal'});
  const [pay,setPay]=useState({month:String(new Date().getMonth()+1).padStart(2,'0'),year:String(new Date().getFullYear())});
  const [payFiles,setPayFiles]=useState([]); const [payMap,setPayMap]=useState(null);
  const [timeDraft,setTimeDraft]=useState({date:todayStr(),task_name:'',task_id:'',project_id:'',hours:'1',minutes:'0',start_time:'',end_time:'',comments:'',billable:true});
  const updateData=useCallback(next=>{setData(prev=>{const raw=typeof next==='function'?next(prev):next;const safe={...seed,...(raw||{})};ptInstantCacheSet(cacheKey,{data:safe,ts:Date.now()});return safe;});},[cacheKey]);
  const mergeServerData=useCallback(d=>{updateData(prev=>{
    let local=null;try{local=JSON.parse(localStorage.getItem(localAttendKey)||'null')}catch(_){ }
    const server=d.attendance_today||null; const lt=local?Date.parse(local.updated||local._localUpdatedAt||0):0; const st=server?Date.parse(server.updated||server.created||0):0;
    const attendance_today=(local&&local.work_date===todayStr()&&lt>=st)?local:server;
  ...
  const people=(data.org_people&&data.org_people.length?data.org_people:users||[]).map(p=>{
    const base={...p,manager_id:p.manager_id||''};
    return cu&&String(base.id)===String(cu.id)?ptMergeProfileNonEmpty(base,cu):base;
  });
  const byId={};people.forEach(p=>byId[String(p.id)]=p);
  const children={};people.forEach(p=>{const mid=String(p.manager_id||'');if(mid&&byId[mid]&&String(mid)!==String(p.id)){(children[mid]=children[mid]||[]).push(p);}});
  let roots=people.filter(p=>!p.manager_id||!byId[String(p.manager_id)]||String(p.manager_id)===String(p.id));
  if(roots.length>1){const admins=roots.filter(p=>String(p.role||'').toLowerCase().includes('admin')||String(p.role||'').toLowerCase().includes('owner'));roots=admins.length?admins:roots.slice(0,1);}
  const directCount=id=>(children[String(id)]||[]).length;
  ...
  const css=html`<style>
    .wos-wrap{--wos-bg:var(--bg,#0b0b0d);--wos-card:var(--sf,#202124);--wos-soft:var(--sf2,#2b2b30);--wos-line:var(--bd,#383842);--wos-tx:var(--tx,#f4f4f5);--wos-muted:var(--tx3,#a1a1aa);--wos-ac:var(--ac,#7c3aed);width:100%;max-width:none;margin:0;padding:18px 24px 48px;color:var(--wos-tx);min-height:calc(100vh - 96px);overflow:auto}
    .wos-top{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:14px}.wos-title h1{margin:0;font-size:28px;letter-spacing:-.04em}.wos-title p{margin:4px 0 0;color:var(--wos-muted);font-size:13px}.wos-actions{display:flex;gap:8px;align-items:center}.wos-pill{border:1px solid var(--wos-line);background:var(--wos-soft);border-radius:999px;padding:7px 11px;font-size:12px;font-weight:800}.wos-live{background:rgba(34,197,94,.14);border-color:rgba(34,197,94,.35);color:#22c55e}
    .wos-tabs{display:flex;gap:8px;flex-wrap:wrap;background:var(--wos-card);border:1px solid var(--wos-line);padding:8px;border-radius:18px;margin:0 auto 14px;max-width:1480px;position:sticky;top:0;z-index:5}.wos-tab{border:0;background:transparent;color:var(--wos-muted);font-weight:900;font-size:12px;padding:10px 14px;border-radius:12px;cursor:pointer}.wos-tab.active{background:var(--wos-ac);color:white;box-shadow:0 10px 24px color-mix(in srgb,var(--wos-ac) 35%,transparent)}
    .wos-grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:16px;max-width:1480px;margin-left:auto;margin-right:auto}.wos-card{background:var(--wos-card);border:1px solid var(--wos-line);border-radius:20px;padding:18px;box-shadow:0 16px 50px rgba(0,0,0,.10);min-width:0}.wos-card h3{margin:0;font-size:16px}.wos-card p{margin:4px 0 0;color:var(--wos-muted);font-size:12px;line-height:1.45}.span-12{grid-column:span 12}.span-8{grid-column:span 8}.span-6{grid-column:span 6}.span-4{grid-column:span 4}.span-3{grid-column:span 3}
    .wos-metric{background:var(--wos-soft);border:1px solid var(--wos-line);border-radius:16px;padding:14px}.wos-metric b{display:block;font-size:24px;letter-spacing:-.04em}.wos-metric small{color:var(--wos-muted);font-weight:800}.wos-row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.wos-form{display:grid;grid-template-columns:repeat(12,1fr);gap:10px;margin-top:12px}.wos-form .inp,.wos-form .sel,.wos-form textarea,.wos-input{background:var(--wos-soft);border:1px solid var(--wos-line);color:var(--wos-tx);border-radius:12px;padding:10px 12px;min-height:40px;min-width:0}.wos-form textarea{grid-column:span 12;min-height:76px}.wos-form .col1{grid-column:span 1}.wos-form .col2{grid-column:span 2}.wos-form .col3{grid-column:span 3}.wos-form .col4{grid-column:span 4}.wos-form .col5{grid-column:span 5}.wos-form .col6{grid-column:span 6}.wos-form .col8{grid-column:span 8}.wos-form .col12{grid-column:span 12}.wos-btn{border:1px solid var(--wos-line);background:var(--wos-soft);color:var(--wos-tx);border-radius:999px;padding:9px 14px;font-weight:900;cursor:pointer}.wos-btn.primary{background:var(--wos-ac);border-color:var(--wos-ac);color:white}.wos-btn.danger{color:#fb7185;border-color:rgba(251,113,133,.35)}
    .wos-table{width:100%;border-collapse:separate;border-spacing:0 6px;margin-top:12px}.wos-table th{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--wos-muted);text-align:left;padding:0 10px}.wos-table td{background:var(--wos-soft);border-top:1px solid var(--wos-line);border-bottom:1px solid var(--wos-line);padding:10px;font-size:12px}.wos-table td:first-child{border-left:1px solid var(--wos-line);border-radius:12px 0 0 12px}.wos-table td:last-child{border-right:1px solid var(--wos-line);border-radius:0 12px 12px 0}.wos-empty{border:1px dashed var(--wos-line);background:var(--wos-soft);border-radius:16px;padding:28px;text-align:center;color:var(--wos-muted);margin-top:12px}.wos-badge{display:inline-flex;align-items:center;border:1px solid color-mix(in srgb,var(--wos-ac) 45%,var(--wos-line));background:color-mix(in srgb,var(--wos-ac) 12%,transparent);color:var(--wos-tx);border-radius:999px;padding:4px 8px;font-size:11px;font-weight:900}
    .wos-org{overflow:auto;padding:18px;min-height:460px;max-height:calc(100vh - 310px)}.wos-tree{display:flex;justify-content:center;min-width:760px}.wos-node{text-align:center;position:relative;padding-top:18px}.wos-person{display:inline-flex;gap:10px;align-items:center;text-align:left;min-width:220px;max-width:260px;background:var(--wos-soft);border:1px solid var(--wos-line);border-radius:16px;padding:11px 12px;color:var(--wos-tx);box-shadow:0 10px 28px rgba(0,0,0,.12);cursor:pointer}.wos-person.active{border-color:var(--wos-ac);box-shadow:0 0 0 3px color-mix(in srgb,var(--wos-ac) 22%,transparent)}.wos-avatar{width:38px;height:38px;border-radius:14px;background:linear-gradient(135deg,var(--wos-ac),#06b6d4);display:grid;place-items:center;color:#fff;font-weight:1000;flex:0 0 auto}.wos-person b{display:block;font-size:12px}.wos-person em{display:block;font-style:normal;color:var(--wos-muted);font-size:11px;margin-top:2px}.wos-person small{display:block;color:var(--wos-muted);font-size:10px;margin-top:3px}.wos-children{display:flex;justify-content:center;gap:16px;position:relative;padding-top:28px}.wos-children:before{content:"";position:absolute;top:12px;left:50%;width:1px;height:16px;background:var(--wos-line)}.wos-children>.wos-node:before{content:"";position:absolute;top:0;left:50%;width:1px;height:18px;background:var(--wos-line)}.wos-children>.wos-node:after{content:"";position:absolute;top:0;left:0;right:0;height:1px;background:var(--wos-line)}.wos-children>.wos-node:first-child:after{left:50%}.wos-children>.wos-node:last-child:after{right:50%}.wos-children>.wos-node:only-child:after{display:none}
    .wos-directory{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:10px;margin-top:14px}.wos-dir-card{display:flex;gap:10px;align-items:flex-start;background:var(--wos-soft);border:1px solid var(--wos-line);border-radius:16px;padding:12px;cursor:pointer}.wos-dir-card b{display:block;font-size:12px}.wos-dir-card small{display:block;color:var(--wos-muted);font-size:11px;line-height:1.35;margin-top:2px}
    .wos-drawer{position:fixed;right:18px;top:78px;bottom:18px;width:min(960px,calc(100vw - 42px));background:var(--wos-card);border:1px solid var(--wos-line);border-radius:22px;padding:16px;z-index:60;box-shadow:0 30px 90px rgba(0,0,0,.45);overflow:auto}.wos-dhead{display:flex;gap:12px;align-items:flex-start;justify-content:space-between}.wos-profile-title{display:flex;gap:12px;align-items:center}.wos-profile-title .wos-avatar{width:52px;height:52px;border-radius:18px}.wos-profile-tabs{display:flex;gap:6px;margin:14px 0;flex-wrap:wrap}.wos-profile-tabs button{border:1px solid var(--wos-line);background:var(--wos-soft);color:var(--wos-muted);border-radius:999px;padding:7px 10px;font-size:11px;font-weight:900}.wos-profile-tabs button.active{background:var(--wos-ac);color:#fff}.wos-field-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.wos-field{background:var(--wos-soft);border:1px solid var(--wos-line);border-radius:14px;padding:11px}.wos-field label{display:block;color:var(--wos-muted);font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:900}.wos-field div{font-size:13px;margin-top:5px;font-weight:800;word-break:break-word}.wos-edit-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.wos-edit-grid .inp,.wos-edit-grid .sel{background:var(--wos-soft);border:1px solid var(--wos-line);color:var(--wos-tx);border-radius:12px;padding:10px 12px}.wos-overlay{position:fixed;inset:0;background:rgba(0,0,0,.25);z-index:55}
  ...
  const attend=data.attendance_today;
  const holidays=data.holidays||[];
  const leaves=data.leaves||[];
  const payslips=data.payslips||[];
  const leaveBalances=data.leave_balances||[];
  const usedLeave=leaves.filter(x=>String(x.status||'').toLowerCase()==='approved').reduce((a,x)=>a+(Number(x.days)||0),0);
  const pendingLeave=leaves.filter(x=>String(x.status||'').toLowerCase()==='pending').reduce((a,x)=>a+(Number(x.days)||0),0);
  const totalLeave=leaveBalances.reduce((a,x)=>a+(Number(x.opening)||0),0)||24;
  const availableLeave=Math.max(0,totalLeave-usedLeave-pendingLeave);
  ...
  const monthDays=useMemo(()=>{
    ...
    const arr=[]; for(let i=0;i<firstDow;i++)arr.push(null);
    for(let d=1;d<=days;d++){const date=y+'-'+String(m+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');arr.push({day:d,date,dow:new Date(y,m,d).getDay()});}
    return arr;
  },[data.generated_at]);
  const attByDate=useMemo(()=>{const m={};(data.attendance_history||[]).forEach(a=>{m[String(a.work_date||'').slice(0,10)]=a});if(attend&&attend.work_date)m[String(attend.work_date).slice(0,10)]=attend;return m;},[data.attendance_history,attend]);
  const holidayByDate=useMemo(()=>{const m={};(holidays||[]).forEach(h=>{m[String(h.holiday_date||'').slice(0,10)]=h});return m;},[holidays]);
  const leaveByDate=useMemo(()=>{const m={};(leaves||[]).forEach(l=>{try{let d=new Date(String(l.start_date).slice(0,10)+'T00:00:00');const end=new Date(String(l.end_date||l.start_date).slice(0,10)+'T00:00:00');while(d<=end){const k=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');m[k]=l;d.setDate(d.getDate()+1);}}catch(_){}});return m;},[leaves]);
  const pending=(data.approvals||[]).filter(x=>String(x.status||'pending')==='pending');
  const profileField=(label,val)=>html`<div class="wos-field"><label>${label}</label><div>${val||'—'}</div></div>`;
  const canEditSelectedProfile=!!(selected&&(canPeopleAdmin||String(selected.id)===String(cu&&cu.id)));
  const timelogs=data.timelogs||[];
  const todayLogs=timelogs.filter(x=>String(x.date||'').slice(0,10)===todayStr());
  const todayMinutes=todayLogs.reduce((a,x)=>a+(Number(x.hours||0)*60)+(Number(x.minutes||0)),0);
  const billableMinutes=todayLogs.filter(x=>x.billable!==0&&x.billable!==false).reduce((a,x)=>a+(Number(x.hours||0)*60)+(Number(x.minutes||0)),0);
  ...
  const workItems=useMemo(()=>{
    ...
    const push=(x,type)=>{
      ...
      const title=String(x.title||x.task_name||x.name||rawId||'').trim();
      const id=rawId||('manual:'+title);
    ...
    // Backend already scopes this to the current user's assigned/current-project work.
    (data.work_items||[]).forEach(x=>push(x,x.item_type||'task'));
    // Fallback for older bootstrap payloads; still avoid random activity catalog options.
    (data.tasks||[]).forEach(x=>push(x,'task'));
    (data.tickets||[]).forEach(x=>push(x,'ticket'));
    return arr.sort((a,b)=>String(b.created||'').localeCompare(String(a.created||'')));
  },[data.work_items,data.tasks,data.tickets]);
  const currentMonthDays=(()=>{const now=new Date();const y=now.getFullYear(),m=now.getMonth();const days=new Date(y,m+1,0).getDate();const by={};timelogs.forEach(x=>{const dt=String(x.date||'').slice(0,10);if(!dt)return;by[dt]=(by[dt]||0)+(Number(x.hours||0)*60)+(Number(x.minutes||0));});return Array.from({length:days},(_,i)=>{const d=new Date(y,m,i+1);d.setMinutes(d.getMinutes()-d.getTimezoneOffset());const key=d.toISOString().slice(0,10);return {day:i+1,key,mins:by[key]||0,today:key===todayStr()};});})();
  const drawer=selected?html`<div class="wos-overlay" onClick=${()=>setSelected(null)}></div><aside class="wos-drawer">
    <div class="wos-dhead"><div class="wos-profile-title"><${Av} u=${selected} size=${52}/><div><h3 style=${{margin:0}}>${selected.name||selected.email}</h3><p style=${{margin:'3px 0 0',color:'var(--wos-muted)',fontSize:12}}>${selected.email}</p><span class="wos-badge">${selected.designation||titleCase(selected.role)||'Team member'}</span></div></div><button class="wos-btn" onClick=${()=>setSelected(null)}>×</button></div>
    <div class="wos-profile-tabs"><button class=${drawerTab==='overview'?'active':''} onClick=${()=>setDrawerTab('overview')}>Overview</button><button class=${drawerTab==='personal'?'active':''} onClick=${()=>setDrawerTab('personal')}>Personal Details</button><button class=${drawerTab==='employment'?'active':''} onClick=${()=>setDrawerTab('employment')}>Employment Details</button><button class=${drawerTab==='documents'?'active':''} onClick=${()=>setDrawerTab('documents')}>Documents</button>${canEditSelectedProfile?html`<button onClick=${()=>{setEditing(!editing);setProfileDraft({...selected});}}>${editing?'Cancel edit':'Edit profile'}</button>`:null}</div>
    ${editing?html`<div style=${{position:'sticky',top:-16,zIndex:2,display:'flex',justifyContent:'flex-end',gap:8,margin:'-4px 0 12px',padding:'10px 0',background:'linear-gradient(180deg,var(--wos-card),color-mix(in srgb,var(--wos-card) 88%,transparent))'}}><button class="wos-btn" onClick=${()=>{setEditing(false);setProfileDraft({...selected});}}>Cancel</button><button class="wos-btn primary" onClick=${saveProfile}>Save profile</button></div><div class="wos-edit-grid">${canPeopleAdmin?html`<input class="inp" placeholder="Employee ID" value=${profileDraft.employee_id||''} onInput=${e=>setProfileDraft({...profileDraft,employee_id:e.target.value})}/><input class="inp" placeholder="Department" value=${profileDraft.department||''} onInput=${e=>setProfileDraft({...profileDraft,department:e.target.value})}/><input class="inp" placeholder="Designation" value=${profileDraft.designation||''} onInput=${e=>setProfileDraft({...profileDraft,designation:e.target.value})}/><select class="sel" value=${profileDraft.role||''} onChange=${e=>setProfileDraft({...profileDraft,role:e.target.value})}>${roleOptions.map(r=>html`<option value=${r}>${r}</option>`)}</select><select class="sel" value=${profileDraft.custom_role||''} onChange=${e=>setProfileDraft({...profileDraft,custom_role:e.target.value})}><option value="">No custom role</option>${customRoles.map(r=>html`<option value=${r.name}>${r.name}</option>`)}</select><select class="sel" value=${profileDraft.manager_id||''} onChange=${e=>setProfileDraft({...profileDraft,manager_id:e.target.value})}><option value="">No manager / top level</option>${people.filter(p=>p.id!==selected.id).map(p=>html`<option value=${p.id}>${p.name||p.email}</option>`)}</select><select class="sel" value=${profileDraft.hod_user_id||''} onChange=${e=>setProfileDraft({...profileDraft,hod_user_id:e.target.value})}><option value="">No HOD</option>${people.filter(p=>p.id!==selected.id).map(p=>html`<option value=${p.id}>${p.name||p.email}</option>`)}</select><input class="inp" placeholder="Job level" value=${profileDraft.job_level||''} onInput=${e=>setProfileDraft({...profileDraft,job_level:e.target.value})}/><input class="inp" placeholder="Date of joining" type="date" value=${profileDraft.date_of_joining||''} onInput=${e=>setProfileDraft({...profileDraft,date_of_joining:e.target.value})}/>`:null}<input class="inp" placeholder="Location" value=${profileDraft.location||''} onInput=${e=>setProfileDraft({...profileDraft,location:e.target.value})}/><input class="inp" placeholder="Salutation" value=${profileDraft.salutation||''} onInput=${e=>setProfileDraft({...profileDraft,salutation:e.target.value})}/><input class="inp" placeholder="Middle name" value=${profileDraft.middle_name||''} onInput=${e=>setProfileDraft({...profileDraft,middle_name:e.target.value})}/><input class="inp" placeholder="Personal phone" value=${profileDraft.personal_phone||''} onInput=${e=>setProfileDraft({...profileDraft,personal_phone:e.target.value})}/><button class="wos-btn primary" onClick=${saveProfile} style=${{gridColumn:'1/-1'}}>Save profile</button></div>`:html`
      ${drawerTab==='overview'?html`<div class="wos-field-grid">${profileField('Email ID',selected.email)}${profileField('Role',titleCase(selected.custom_role||selected.role))}${profileField('Department',selected.department||'Unassigned')}${profileField('Designation',selected.designation)}${profileField('Manager',byId[String(selected.manager_id||'')]&&byId[String(selected.manager_id||'')].name)}${profileField('Direct Reports',directCount(selected.id))}${profileField('Location',selected.location)}${profileField('Employment Type',selected.employment_type||'full-time')}</div>`:null}
      ${drawerTab==='personal'?html`<div class="wos-field-grid">${profileField('First Name',String(selected.name||'').split(' ')[0])}${profileField('Last Name',String(selected.name||'').split(' ').slice(1).join(' '))}${profileField('Middle Name',selected.middle_name)}${profileField('Salutation',selected.salutation)}${profileField('Date of Birth',canHR||selected.dob_visibility==='everyone'||String(selected.id)===String(cu&&cu.id)?(selected.dob||selected.birthday):'Private')}${profileField('DOB Access',selected.dob_visibility||'Private')}</div>`:null}
      ${drawerTab==='employment'?html`<div class="wos-field-grid">${profileField('Employee ID',selected.employee_id)}${profileField('Date of Joining',selected.date_of_joining)}${profileField('Band / Grade',selected.band)}${profileField('Functional Area',selected.functional_area)}${profileField('Job Level',selected.job_level)}${profileField('HOD',byId[String(selected.hod_user_id||'')]&&byId[String(selected.hod_user_id||'')].name)}${profileField('Created',fmtDate(selected.created))}</div>`:null}
      ${drawerTab==='documents'?html`<div class="wos-empty">${String(selected.id)===String(cu&&cu.id)||canHR?'Documents and payslips for this employee will appear here based on permissions.':'You do not have access to this employee documents.'}</div>`:null}
    `}
  </aside>`:null;
  return html`${css}<div class="wos-wrap">
    ...
    ${tab==='ann'?html`<div class="wos-grid"><div class="wos-card span-12"><div class="wos-row" style=${{justifyContent:'space-between'}}><div><h3>Announcements</h3><p>Workspace updates with instant posting.</p></div>${canPeopleAdmin?html`<button class="wos-btn primary" onClick=${postAnnouncement}>Post</button>`:null}</div>${canPeopleAdmin?html`<div class="wos-form"><input class="inp col4" placeholder="Title" value=${ann.title} onInput=${e=>setAnn({...ann,title:e.target.value})}/><select class="sel col3" value=${ann.priority} onChange=${e=>setAnn({...ann,priority:e.target.value})}><option>normal</option><option>important</option><option>urgent</option></select><textarea class="inp" placeholder="Message" value=${ann.body} onInput=${e=>setAnn({...ann,body:e.target.value})}></textarea></div>`:null}${(data.announcements||[]).length?html`<table class="wos-table"><tbody>${(data.announcements||[]).map(a=>html`<tr><td><b>${a.title}</b><br/><small>${a.body}</small></td><td>${titleCase(a.priority)}</td><td>${fmtDate(a.created)}</td></tr>`)}</tbody></table>`:html`<div class="wos-empty">No announcements yet.</div>`}</div></div>`:null}
    ${tab==='approvals'?html`<div class="wos-grid"><div class="wos-card span-12"><h3>Workspace approvals</h3><p>Leave, attendance regularisation and future HR/work requests.</p>${pending.length?html`<table class="wos-table"><thead><tr><th>Request</th><th>Summary</th><th>Status</th><th>Created</th></tr></thead><tbody>${pending.map(a=>html`<tr><td>${titleCase(a.request_type)}</td><td>${a.summary||a.title}</td><td>${titleCase(a.status)}</td><td>${fmtDate(a.created)}</td></tr>`)}</tbody></table>`:html`<div class="wos-empty">No pending approvals in your queue.</div>`}</div></div>`:null}
    ${drawer}
  </div>`;
}
...
function App(){
  ...
  const [data,setData]=useState(()=>{
    if(initialAppCache&&!initialAppCache.error){
      ...
      return {
        users:Array.isArray(initialAppCache.users)?initialAppCache.users:[],
        projects:(Array.isArray(initialAppCache.projects)?initialAppCache.projects:[]).map(p=>({...p,members:_pm(p.members)})),
        tasks:Array.isArray(initialAppCache.tasks)?initialAppCache.tasks:[],
        notifs:Array.isArray(initialAppCache.notifications)?initialAppCache.notifications:[],
        teams:Array.isArray(initialAppCache.teams)?initialAppCache.teams:[],
        tickets:Array.isArray(initialAppCache.tickets)?initialAppCache.tickets:[],
        featureFlags:initialAppCache.feature_flags||{},featureCatalog:initialAppCache.feature_catalog||{},
        workspacePlan:initialAppCache.plan||'starter',planLimits:initialAppCache.plan_limits||{},dashboardSummary:initialAppCache.dashboardSummary||{}
      };
    }
    return {users:[],projects:[],tasks:[],notifs:[],teams:[],tickets:[],featureFlags:{},featureCatalog:{},workspacePlan:"starter",planLimits:{},dashboardSummary:{}};
  });
  ...
  const routeToNotification=useCallback(async(n={})=>{
    ...
    if((looksDm||looksCall)){
      if(!dmPeer){
        ...
        // Capture current URL before async call so we can detect if user navigated away.
        const _navBefore=window.location.href;
        try{
          dmPeer=s(await ptResolveDmNotificationTarget(cu&&cu.id, dmUnread));
        }catch(_e){}
        // If user navigated somewhere else while we were awaiting, abort DM routing.
        if(window.location.href!==_navBefore && !window.location.pathname.includes('/dm')){return;}
      }
          ...
          try{
            ...
            const __ptMsgForMe=(m)=>{
              try{
                ...
                const users=Array.isArray(d.users)?d.users.map(String):[];
                ...
                if(!me)return false;
                if(users.length)return users.includes(me);
  ...
  const load=useCallback(async(overrideTeamCtx, bust=false)=>{
    ...
    try{
      ...
      if(cachedApp&&!cachedApp.error){
        ...
        if(cachedApp.partial){
          setData(prev=>({...prev,featureFlags:Object.keys(featureFlags||{}).length?featureFlags:prev.featureFlags,featureCatalog:Object.keys(featureCatalog||{}).length?featureCatalog:prev.featureCatalog,workspacePlan:workspacePlan||prev.workspacePlan||'starter',planLimits:Object.keys(planLimits||{}).length?planLimits:prev.planLimits}));
        }else{
      ...
      if(d&&d.partial){
        ...
        setData(prev=>({...prev,featureFlags:Object.keys(featureFlags).length?featureFlags:prev.featureFlags,featureCatalog:Object.keys(featureCatalog).length?featureCatalog:prev.featureCatalog,workspacePlan,planLimits}));
        if(ws&&ws.name)setWsName(ws.name);
        if(ws){try{window.PT_WORKSPACE=Object.assign({},window.PT_WORKSPACE||{},ws,{workspace_id:ws.id||ws.workspace_id||ws.workspace_id_from_me||((cu&&cu.workspace_id)||''),workspace_id_from_me:ws.id||ws.workspace_id||ws.workspace_id_from_me||((cu&&cu.workspace_id)||''),workspace_slug:ws.workspace_slug||ws.slug||'',workspace_name:ws.name||ws.workspace_name||''});}catch(_){} setWsDmEnabled(ws.dm_enabled!==0);}
        return;
      }
  ...
  useEffect(()=>{
    const h=(e)=>{
      ...
      if(e.shiftKey&&e.key==='L'&&!e.metaKey&&!e.ctrlKey){const tag=document.activeElement&&document.activeElement.tagName;if(tag!=='INPUT'&&tag!=='TEXTAREA'){e.preventDefault();_setView('workspace-os:time');}}
  ...
  const VIEW_FEATURE_MAP={'workspace-os':'employee_self_service',timesheet:'time_tracking',billing:'billing_invoices','ai-docs':'ai_docs',vault:'vault',productivity:'advanced_analytics',ops:'advanced_analytics',tickets:'tickets'};
  ...
  return html`
    ...
    ${globalIncomingCall?html`<div style=${{position:'fixed',inset:0,zIndex:100000,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(2,6,23,.70)',backdropFilter:'blur(10px)'}} onClick=${e=>{if(e.target===e.currentTarget){}}}>
      <div style=${{width:'min(520px,92vw)',borderRadius:32,padding:'34px 28px',textAlign:'center',background:'radial-gradient(circle at 50% 0%,rgba(34,197,94,.18),rgba(15,23,42,.98) 48%,rgba(2,6,23,.98))',border:'1px solid rgba(255,255,255,.14)',boxShadow:'0 35px 110px rgba(0,0,0,.65)',color:'#fff'}}>
        <div style=${{fontSize:12,fontWeight:900,letterSpacing:1.5,textTransform:'uppercase',color:'#93c5fd',marginBottom:18}}>Incoming video call</div>
        ...
        <div style=${{display:'flex',justifyContent:'center',gap:47,marginTop:12,fontSize:12,fontWeight:900,color:'#e2e8f0'}}><span>Disconnect</span><span>Connect</span></div>
      </div>
    </div>`:null}
      ...
      <div style=${{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:9800,display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:'10vh',backdropFilter:'blur(4px)'}}
        ...
        <div style=${{width:'min(640px,92vw)',background:'var(--sf)',borderRadius:16,boxShadow:'0 24px 80px rgba(0,0,0,.35)',border:'1px solid var(--bd)',overflow:'hidden'}}>
          ...
          <div style=${{display:'flex',alignItems:'center',gap:10,padding:'14px 18px',borderBottom:'1px solid var(--bd)'}}>
            ...
            <span style=${{fontSize:9,color:'var(--tx3)',background:'var(--sf2)',padding:'2px 6px',borderRadius:5,border:'1px solid var(--bd)'}}>ESC</span>
          </div>
          <!-- Results -->
