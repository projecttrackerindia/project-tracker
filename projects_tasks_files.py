...
def get_app_data():
    ...
    # Force-refresh, but never block initial page load for 8-11s.
    # The SPA sometimes calls bust=1 during boot, so cap the wait and warm cache
    # in the background if the DB is slow.
    if request.args.get("bust") == "1":
        import threading as _bp_thread, os as _bp_os
        ...
        def _fetch_bust():
            try:
                ...
                holder["err"] = _e
                try: log.warning("[app-data bust fallback] %s", _e)
                except Exception: pass
        t = _bp_thread.Thread(target=_fetch_bust, daemon=True)
        t.start()
        t.join(float(_bp_os.environ.get("APPDATA_BUST_WAIT_SECONDS", "0.75")))
        result = holder.get("val")
        if result is None:
            try:
                cached = _cache_get(cache_key) if '_cache_get' in globals() else None
                if isinstance(cached, dict) and not cached.get("partial"):
                    cached = dict(cached); cached["refreshing"] = True; cached["served_from_bust_cache"] = True
                    return _etag_response(cached)
            except Exception:
                pass
            return jsonify({"users": [], "projects": [], "tasks": [], "notifications": [],
                            "dm_unread": [], "workspace": {"id": ws}, "teams": [], "tickets": [],
                            "reminders": [], "partial": True, "retry_after_ms": 1000,
                            "loading_full_data": True})
        return _etag_response(result)
    ...
    # Cache cold or too stale — give DB a short window, then return a minimal
    # shell and let the background thread warm app-data.
    import threading as _bp_thread, os as _bp_os
    holder = {"val": None}
    def _cold_fetch():
        try:
            holder["val"] = _fetch_app_data_from_db(ws, team_id, uid)
            _cache_set(cache_key, holder["val"])
        except Exception as _e:
            try: log.warning("[blueprint app-data cold] %s", _e)
            except Exception: pass
    t = _bp_thread.Thread(target=_cold_fetch, daemon=True)
    t.start()
    t.join(float(_bp_os.environ.get("APPDATA_COLD_WAIT_SECONDS", "0.75")))
    result = holder.get("val")
    if result is None:
        return jsonify({"users": [], "projects": [], "tasks": [], "notifications": [],
                        "dm_unread": [], "workspace": {"id": ws}, "teams": [], "tickets": [],
...
def get_tasks():
    ...
    if not bust:
        ...
        cached = _cache_get(cache_key)
        if cached is not None: return jsonify(_filter_deleted_tombstones(ws, cached))
    with get_db() as db:
        if team_id:
...
def create_task():
    ...
    assignee_ref = _normalize_assignee_ref(d) if '_normalize_assignee_ref' in globals() else str(d.get("assignee") or '').strip()
    d["assignee"] = assignee_ref
    with get_db() as db:
        ...
        if d.get("assignee"):
            try:
                assignee_user=_resolve_user_for_task_email(db, d.get("assignee"), wid())
            except Exception:
                assignee_user=db.execute("SELECT id,name,email FROM users WHERE id=?",(d["assignee"],)).fetchone()
            if not assignee_user:
                try: log.warning("[TaskEmail] skipped — assignee not found ref=%r task=%s ws=%s", d.get("assignee"), tid, wid())
                except Exception: pass
        proj=None
        proj_members=[]
        if d.get("project"):
        ...
        if assignee_user:
            assignee_email = _clean_email_addr(_row_value(assignee_user, "email", "")) if '_row_value' in globals() else (assignee_user["email"] if assignee_user else "")
            assignee_name = _row_value(assignee_user, "name", "there") if '_row_value' in globals() else assignee_user["name"]
            assignee_real_id = _row_value(assignee_user, "id", d.get("assignee")) if '_row_value' in globals() else d.get("assignee")
            if assignee_email:
                _start_task_assigned_email_thread(
                    assignee_email, assignee_name, d["title"], cname, tid, wid()
                )
            else:
                try: log.warning("[TaskEmail] skipped — resolved assignee has no email ref=%r task=%s ws=%s", d.get("assignee"), tid, wid())
                except Exception: pass
            # FIX 14/19: Route push via job queue (non-blocking, backpressure-aware)
            try:
                _job_queue.enqueue(push_notification_to_user, None, assignee_real_id,
                    f"✅ New task assigned: {d['title']}",
                    f"{cname} assigned you this task [{d.get('priority','medium')}]",
                    f"/?action=task&id={tid}", queue_name="notifications")
            except Exception:
                threading.Thread(target=push_notification_to_user,
                    args=(db,assignee_real_id,f"✅ New task assigned: {d['title']}",
                          f"{cname} assigned you this task [{d.get('priority','medium')}]",f"/?action=task&id={tid}"),
                    daemon=True).start()
        for uid in proj_members:
            if uid==session["user_id"] or uid==d.get("assignee"): continue
            try:
                _job_queue.enqueue(push_notification_to_user, None, uid,
                    f"📋 New task in {proj['name'] if proj else ''}",
                    f"{cname} created '{d['title']}'", "/", queue_name="notifications")
            except Exception:
                threading.Thread(target=push_notification_to_user,
        ...
        try:
            _queue_task_assignment_email_for_created_task(dict(t) if t else tid, session.get("user_id"), wid(), source="blueprint.create_task")
        except Exception as e:
            try: log.exception("[TaskEmail] blueprint dispatch failed task=%s ws=%s: %s", tid, wid(), e)
            except Exception: pass
...
def del_task(tid):
    ...
    with get_db() as db:
        ...
        deleted_ts = ts()
        try:
            db.execute("UPDATE tasks SET deleted_at=? WHERE id=? AND workspace_id=?",(deleted_ts,tid,ws))
        except Exception:
            db.execute("DELETE FROM tasks WHERE id=? AND workspace_id=?",(tid,ws))
    try:
